      ...
      case "xupdate:insert-after":
        switch (nodoEncontrado) {
          ...
          case 1:
            ...
            this.iface.pub_logAdd("Warning: El nodo se insertará como último hijo");
            var nodoAux = nodoDif.firstChild().cloneNode();
      ...
      case "xupdate:append-first":
        switch (nodoEncontrado) {
          ...
          case 2:
            ...
            this.iface.pub_logAdd("Error: Se perdió el nodo actual buscando " + nodoDif.attributeValue("select") + " en " + nomNodo + "-" + rutaNodo[i]);
            return false;
        }
        break;
      case "xupdate:delete":
        switch (nodoEncontrado) {
          case 0:
            ...
            break;
          case 1:
            this.iface.pub_logAdd("Delete: Eliminación de " + nodoDif.attributeValue("select"));
            this.iface.pub_logAdd("Warning: El nodo especificado no existe");
            break;
          case 2:
            this.iface.pub_logAdd("Delete:");
            this.iface.pub_logAdd("Error: Se perdió el nodo actual buscando " + nodoDif.attributeValue("select") + " en " + nomNodo + "-" + idNodo);
            return false;
            break;
        }
        break;
...
function head_aplicarParche(codFuncional: String, dirParche: String, dirDest: String)
{
  ...
  if (!File.exists(dirParche + "/" + codFuncional + ".xml")) {
    this.iface.pub_logAdd("El parche " + codFuncional + " está vacío");
    return true;
  }
...
function head_aplicarParcheNodo(nodo: FLDomNode, dirParche: String, dirOrig: String, dirDest: String)
{
  ...
  /// Si no existe el directorio origen es porque el módulo de turno no ha bajado, porque el cliente no lo tiene aunque la extensión lo toque
  ...
  switch (nodo.nodeName()) {
    ...
    case "flpatch:addFile":
      this.iface.pub_logAdd("Añadiendo " + nombre);
      var comando = "cp -f " + this.iface.pathLocal + dirParche + "/" + nombre + " " + this.iface.pathLocal + dirDest + "/" + pathFichero + nombre;
      var resComando = this.iface.ejecutarComando(comando);
      if (resComando.ok == false) {
        this.iface.pub_logAdd("Error al copiar el fichero con: " + comando);
  ...
  while (qryDep.next()) {
    ...
    lista[indice]["ver"] = version;
		if (qryDep.value("f.proyecto")) {
			/// Cuando se depende de un proyecto se baja el proyecto en su última versión pero el resto de funcionalidades en la versión base del mismo. Esto permite bajar la misma versión de extensiones que las del proyecto del que se hereda, y evita que se bajen todas las dependencias del proyecto en la última versión
...
function head_checkoutMods(codFuncional: String, dirDestino1: String, dirDestino2: String, versionBase: String)
{
  ...
  if (!qryMods.exec()) {
    this.iface.pub_logAdd("Error al ejecutar la consulta de módulos afectados por " + codFuncional);
    return false;
  }
  var dirArea: String;
  var dirMod: String;
  ...
  while (qryMods.next()) {
    ...
    // debug("****Area = " + dirArea);
    // debug("****Mod = " + dirMod);
    this.iface.pub_logAdd("Obteniendo módulo " + dirArea + "/" + dirMod);
