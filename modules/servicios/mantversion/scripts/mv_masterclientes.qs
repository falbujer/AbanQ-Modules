...
function head_generar(idCliente: String)
{
  ...
  flmaveppal.iface.pub_logAdd(AQUtil.translate("scripts", "La funcionalidad del cliente seleccionado ha sido obtenida correctamente"));
}
...
function head_generarDirModulos(idCliente: String): Boolean {
  ...
  if (!listaDep)
  {
    flmaveppal.iface.pub_logAdd("Error al obtener la lista de dependencias de " + idCliente);
    return false;
  }
  ...
  for (var i: Number = 0; i < listaDep.length; i++)
  {
    if (!flmaveppal.iface.pub_checkoutMods(listaDep[i], idCliente, "modulos")) {
      flmaveppal.iface.pub_logAdd("Error al obtener los módulos afectados por la funcionalidad " + listaDep[i]);
      return false;
    }
    ...
    if (!flmaveppal.iface.pub_checkoutParche(listaDep[i], idCliente + "/temp")) {
      flmaveppal.iface.pub_logAdd("Error al obtener el parche " + listaDep[i]);
      return false;
    }
    if (!flmaveppal.iface.pub_aplicarParche(listaDep[i], idCliente + "/temp", idCliente + "/modulos")) {
      flmaveppal.iface.pub_logAdd("Error al aplicar el parche " + listaDep[i]);
      return false;
    }
    ...
    if (!this.iface.anadirAConfig(listaDep[i], idCliente)) {
      flmaveppal.iface.pub_logAdd("Error al añadir las funcionalidad " + listaDep[i] + " al fichero de configuración del cliente");
      return false;
    }
  }
