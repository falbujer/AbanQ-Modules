...
function interna_init()
{
	...
	if (sys.isLoadedModule("flcolaproc")) {
		var datosS = [];
		datosS["tipoObjeto"] = "crm_oportunidadventa";
		datosS["idObjeto"] = cursor.valueBuffer("codoportunidad");
		flcolaproc.iface.pub_seguimientoOn(this, datosS);
	} else {
		this.child("tbwCliente").setTabEnabled("tareas", false);
	}

	connect(this.child("toolButtonPrintPre"), "clicked()", _i, "imprimirPresupuesto()");
	switch (cursor.modeAccess()) {
		case cursor.Insert: {
			var codCliente = cursor.valueBuffer("codcliente");
			if (codCliente && codCliente != "") {
				this.child("fdbNomCliente").setValue(AQUtil.sqlSelect("clientes", "nombre", "codcliente = '" + codCliente + "'"));
			}
			this.child("fdbIdUsuario").setValue(sys.nameUser());
			this.child("fdbCodEstado").setValue(AQUtil.sqlSelect("crm_estadosoportunidad", "codestado", "valordefecto = true"));
		}
	}
}
