...
class ifaceCtx extends head {
	...
	function pub_buscarCampo(codConsulta:String) {
		return this.buscarCampo(codConsulta);
	}
	function pub_buscarCampoTablas(codConsulta:String) {
		return this.buscarCampoTablas(codConsulta);
	}
	function pub_queryLista(codLista:String) {
		return this.queryLista(codLista);
	}
	function pub_seleccionCampo(listaCampos:String) {
		return this.seleccionCampo(listaCampos);
	}
	function pub_arrayAliasCampana(codCampana:String) {
		return this.arrayAliasCampana(codCampana);
	}
	function pub_sustituirAlias(plantilla:String, datos:String, arrayAlias:Array) {
		return this.sustituirAlias(plantilla, datos, arrayAlias);
	}
	function pub_actualizarListaAnalisis(curLista:FLSqlCursor) {
		return this.actualizarListaAnalisis(curLista);
	}
	function pub_valorConfigMarketing(nombreValor:String) {
...
\end */
function oficial_buscarCampo(codConsulta:String)
{
	var util= new FLUtil();
	
	var listaCampos= util.sqlSelect("crm_consultasmark", "campos", "codconsulta = '" + codConsulta + "'");
	if (!listaCampos)
		return false;

	var valores= this.iface.seleccionCampo(listaCampos);
	if (!valores)
		return false;
	return valores;
}
...
o false si hay error
\end */
function oficial_buscarCampoTablas(codConsulta:String)
{
	var util= new FLUtil();

	var res= [];
	var listaTablas= util.sqlSelect("crm_consultasmark", "listatablas", "codconsulta = '" + codConsulta + "'");
	if (!listaTablas)
		return false;

	var xmlTabla:FLDomDocument;
	var nodosCampo= new FLDomNodeList;
...
o false si hay error
\end */
function oficial_seleccionCampo(listaCampos:String)
{
	var util= new FLUtil();
...
/*
	var campo= Input.getItem(util.translate("scripts", "Seleccione campo"), arrayCampos, false, false, util.translate("scripts", "Título"));
	if (!campo)
		return false;
	return campo;
*/
...
\end */
function oficial_sinEspacios(cadena:String)
{
	var resultado= cadena.toString();
	if (!resultado)
		return "";

	var posEspacio= resultado.find(" ");
	while (posEspacio > -1) {
...
\end */
function oficial_queryLista(codLista:String)
{
	var util= new FLUtil;

	var criterios= "";
	var codConsulta= util.sqlSelect("crm_listasmark", "codconsulta", "codlista = '" + codLista + "'");
	var qryConsulta= new FLSqlQuery;
	with(qryConsulta) {
	...
	var dondeConsulta= qryConsulta.value("donde");
	if (dondeConsulta && dondeConsulta != "")
		criterios = "(" + dondeConsulta + ")";

	var qryCriterios= new FLSqlQuery;
	with(qryCriterios) {
	...
	var igualQue= util.translate("scripts", "Igual que");
	var distintoDe= util.translate("scripts", "Distinto de");
	var mayorQue= util.translate("scripts", "Mayor que");
	var menorQue= util.translate("scripts", "Menor que");
	var mayorOIgualQue= util.translate("scripts", "Mayor o igual que");
	var menorOIgualQue= util.translate("scripts", "Menor o igual que");
	var entre= util.translate("scripts", "Entre");
	var en= util.translate("scripts", "Incluido en");
	var noEn= util.translate("scripts", "No incluido en");
	var contiene= util.translate("scripts", "Contiene");
	var noContiene= util.translate("scripts", "No contiene");
	var esNulo = util.translate("scripts", "Es nulo");
	var noEsNulo = util.translate("scripts", "No es nulo");
		...
		switch (qryCriterios.value("tipo")) {
			...
			case 26: {
				switch (qryCriterios.value("condicion")) {
					case en: 
					case noEn: {
						var valor= qryCriterios.value("valor");
	...
	var miFrom= qryConsulta.value("desde");
	var esManual= util.sqlSelect("crm_listasmark", "manual", "codlista = '" + codLista + "'");
	if (esManual) {
...
\end */
function oficial_arrayAliasCampana(codCampana:String)
{
	var arrayAlias= [];
...
\end */
function oficial_sustituirAlias(plantilla:String, datos:String, arrayAlias:Array)
{
	...
	for (var i= 0; i < arrayAlias.length; i++) {
...
\end */
function oficial_actualizarListaAnalisis(curLista:FLSqlCursor)
{
	var util= new FLUtil();

	var codLista= curLista.valueBuffer("codlista");
	var posicion= curLista.valueBuffer("posanalisis");
	var cubo= curLista.valueBuffer("cuboanalisis");
...
function oficial_valorConfigMarketing(nombreValor:String)
{
	var util= new FLUtil;
	var valor= util.sqlSelect("crm_configmark", nombreValor, "1 = 1");
	return valor;
}
//// OFICIAL /////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
