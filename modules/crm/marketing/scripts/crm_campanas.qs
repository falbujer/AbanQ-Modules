...
class oficial extends interna {
	...
	function generarLista(cursor:FLSqlCursor) {
		return this.ctx.oficial_generarLista(cursor);
	}
	function valoresAlias(idLista:String) {
	...
	function enviarEmailCampana(curDestinatario:FLSqlCursor, arrayAlias:Array) {
	...
	function crearComunicacionCampana(curDestinatario:FLSqlCursor, canal:String) {
	...
	function dameResponsableDestinatario(curDestinatario:FLSqlCursor) {
...
\end */
function oficial_generarLista(cursor:FLSqlCursor)
{
	var util= new FLUtil();
	var codCampana= cursor.valueBuffer("codcampana");

	if (!util.sqlDelete("crm_destinacampana", "codcampana = '" + codCampana + "'"))
		return false;

	var curDestina= new FLSqlCursor("crm_destinacampana");

	var qryListas= new FLSqlQuery;
	with (qryListas) {
...
debug(qryListas.sql());
	...
	var progreso:Number;
	var canal= cursor.valueBuffer("canal");
	var plantilla= cursor.valueBuffer("plantiemail");
	...
	while (qryListas.next()) {
		...
		if (qryListas.value("lm.importanalisis")) {
			var res= MessageBox.warning(util.translate("scripts", "La lista %1 está asociada al Módulo de Análisis.\n¿Desea recargar esta lista con los datos actuales de dicho módulo?").arg(codLista), MessageBox.Yes, MessageBox.No);
			if (res == MessageBox.Yes) {
				var curLista= new FLSqlCursor("crm_listasmark");
...
\end */
function oficial_valoresAlias(idLista:String)
{
	var arrayAlias= [];
...
{
	var util= new FLUtil;
	var cursor= this.child("tdbListasCampana").cursor();
	var codCampana= cursor.valueBuffer("codCampana");
	var orden= cursor.valueBuffer("orden");
	var orden2:Number;
	var fila= this.child("tdbListasCampana").currentRow();
	...
	var curLista= new FLSqlCursor("crm_listascampana");
...
function oficial_lanzarCampanaTelefono()
{
	...
	MessageBox.information(util.translate("scripts", "Procesa la campaña de forma manual desde la opción Destinatarios de Campaña"), MessageBox.Ok, MessageBox.NoButton);
	return;
...
// 		return true;
// 	}
// 	var res= MessageBox.information(util.translate("scripts", "¿Desea crear un registro de comunicación en estado Borrador para cada destinatario?"), MessageBox.Yes, MessageBox.No);
// 	if (res != MessageBox.Yes) {
// 		return true;
// 	}
// 
// 	var curDestinatarios= new FLSqlCursor("crm_destinacampana");
// 	curDestinatarios.select("codcampana = '" + codCampana + "' AND codestado <> 'ENVIADO'");
// 
// 	var envios= 0;
// 	var limite= curDestinatarios.size();
...
function oficial_dameResponsableDestinatario(curDestinatario:FLSqlCursor)
{
	var util= new FLUtil;
	var campoClave= curDestinatario.valueBuffer("campoclave");
	if (!campoClave || campoClave == "") {
		return "";
	}
	var aCC= campoClave.split(".");
	var tabla= aCC[0];
debug("tabla " + tabla);
...
function oficial_crearComunicacionCampana(curDestinatario:FLSqlCursor, canal:String)
{
	var util= new FLUtil;

	var curCampana= this.cursor();
	var curComunicacion= new FLSqlCursor("crm_comunicaciones");
	var codCampana= curDestinatario.valueBuffer("codcampana");
	var idDest= curDestinatario.valueBuffer("iddestinatario");
	curComunicacion.select("codcampana = '" + codCampana + "' AND iddestinatario = " + idDest);
	if (curComunicacion.first()) {
		return true;
	}
	var hoy= new Date;
	var codComunicacion= flcrm_ppal.iface.pub_siguienteSecuencia("crm_comunicaciones", "codcomunicacion", 10);
...
function oficial_lanzarCampanaEmail()
{
	...
	var arrayAlias= flcrm_mark.iface.pub_arrayAliasCampana(codCampana);
	if (!arrayAlias) {
		return false;
	}
	
	var limite= Input.getNumber(util.translate("scripts", "Limite de envios (0 para lanzarlos todos)"));
	if (isNaN(limite)) {
		return false;
	}
...
function oficial_probarEnvio()
{
	...
	var emailDefecto= util.sqlSelect("usuarios", "email", "idusuario = '" + sys.nameUser() + "'");
	if (!emailDefecto)
		emailDefecto = "";
	var destino= Input.getText(util.translate("scripts", "Indique E-mail del destinatario del correo de prueba"), emailDefecto, util.translate("scripts", "Prueba de envío"));
	if (!destino)
		return false;

	var curDestinatario= this.child("tdbDestinatariosCamp").cursor();
	...
	var arrayAlias= flcrm_mark.iface.pub_arrayAliasCampana(cursor.valueBuffer("codCampana"));
	if (!arrayAlias)
		return false;
...
|end */
function oficial_enviarEmailCampana(curDestinatario:FLSqlCursor, arrayAlias:Array)
{
	var util= new FLUtil;
	var cursor= this.cursor();
	
	var rutaPlantilla= cursor.valueBuffer("planticorreo");
	var formatoHtml= (rutaPlantilla && rutaPlantilla != "" && rutaPlantilla.toLowerCase().endsWith(".html"));
	
	var correo= new FLSmtpClient;
	var dirCorreSaliente= flcrm_mark.iface.pub_valorConfigMarketing("hostcorreosaliente");
	if (!dirCorreSaliente || dirCorreSaliente == "") {
		dirCorreSaliente = "localhost";
	}
	correo.setMailServer(dirCorreSaliente);
	var remite= cursor.valueBuffer("remiteemail");
	if (!remite || remite == "") {
	...
	var asunto= cursor.valueBuffer("descripcion");
...
function oficial_lanzarCampanaCorreo()
{
	...
	var codCampana= cursor.valueBuffer("codcampana");
	var ahora= new Date();
	var dirDestino= cursor.valueBuffer("codcampana") + "-" + ahora.toString();
	
	dirDestino = dirDestino.replace(":", "-");
	dirDestino = dirDestino.replace(":", "-");
	var dirTemp= "borrame_" + dirDestino;
	...
	var resComando= this.iface.ejecutarComando("unzip ../" + fichPlantilla.name);
	if (resComando.ok == false) {
		return false;
	}
	var codificacion= util.readSettingEntry("scripts/flfacturac/encodingLocal");

	var plantilla= File.read(fichPlantilla.path + "/" + dirTemp + "/content.xml");
	if (!plantilla)
		return false;
	...
	while (curDestinatarios.next()) {
		...
		var campoClave= curDestinatarios.valueBuffer("campoclave");
...
function oficial_tbnEtiDireccionClicked()
{
	var util= new FLUtil;
	var cursor= this.cursor();
	var nombresReport= ["Apli 70x35mm", "Apli 105x48mm"];
	...
	var qryDestinos= new FLSqlQuery;
	with (qryDestinos) {
	...
	var etiquetaInicial= flfactinfo.iface.seleccionEtiquetaInicial();
...
function oficial_tbnCampoPlanClicked()
{
	...
	var arrayAlias= flcrm_mark.iface.pub_arrayAliasCampana(codCampana);
	var seleccion= Input.getItem(util.translate("scripts", "Seleccione alias"), arrayAlias);
	if (!seleccion)
		return;
...
function oficial_habilitarPlantillaEmail()
{
	...
	switch (canal) {
		case "E-mail": {
			var plantiCorreo= cursor.valueBuffer("planticorreo");
...
function oficial_exportar()
{
	...
	var fichero= FileDialog.getSaveFileName("*.csv", "Seleccionar fichero...")
	if (!fichero) {
		return;
	}
	var file = new File(fichero);
	file.open(File.WriteOnly);
	var contenido= this.iface.generarFichero();
	file.write(contenido);
	file.close();
	MessageBox.information(util.translate("scripts", "Generado fichero en :\n\n" + fichero + "\n\n"), MessageBox.Ok, MessageBox.NoButton);
}

function oficial_generarFichero()
{
	var util= new FLUtil();
	var cursor = this.cursor();
	var contenido = "";

	var opciones= [util.translate("scripts", "Intro"), util.translate("scripts", "Tabulador"), util.translate("scripts", "Otro")];
	var titulo= util.translate("scripts", "Seleccione caracter separador");
	...
	var chkIncluirAlias = new CheckBox;
	chkIncluirAlias.text = util.translate("scripts", "Incluir datos de alias");
	dialog.add(chkIncluirAlias);
	...
	var qryDestino= new FLSqlQuery;
	qryDestino.setTablesList("crm_destinacampana");
	qryDestino.setSelect("destino, datos");
	...
	var codCampana = cursor.valueBuffer("codcampana");
	var aAlias = flcrm_mark.iface.pub_arrayAliasCampana(codCampana);
	var totalAlias = aAlias ? aAlias.length : 0;
	var destino, datos;
	var xmlDatos = new FLDomDocument;
	var eDatos;
	while(qryDestino.next()) {
		destino = qryDestino.value("destino");
		if (destino && destino != "") {
			while (destino.find("\n") >= 0) {
		...
		contenido += destino;
		datos = qryDestino.value("datos");
		if (totalAlias > 0 && datos && datos != "") {
			if (xmlDatos.setContent(datos)) {
				eDatos = xmlDatos.firstChild().toElement();
				for (var i = 0; i < totalAlias; i++) {
					contenido += caracter + eDatos.attribute(aAlias[i]) ;
				}
			}
		}
		contenido += chkIncluirAlias.checked ? "\n" : caracter;
	}
	return contenido;
}
