...
function interna_init()
{
	...
	this.iface.CUOTAPATRONAL = 6;
	this.iface.DIETAS = 7;
	this.iface.SNETO = 8;
	this.iface.INCLUIR = 9;
	
	tblNominas.setNumCols(10);
	...
	tblNominas.setColumnWidth(this.iface.CUOTAPATRONAL, 95);
	...
	tblNominas.setColumnLabels("/", "Código/Nombre/Apellidos/S. Bruto/IRPF/Seg. Social/Cuota patronal/Dietas/S. Neto/Incluir");
...
function oficial_actualizar()
{
	...
	while (curEmpleados.next()) {
		...
		curNomina.setValueBuffer("cuotapatronalss", formRecordrh_nominas.iface.pub_commonCalculateField("cuotapatronalss", curNomina));
		tblNominas.setText(0, this.iface.CUOTAPATRONAL, curNomina.valueBuffer("cuotapatronalss"));
