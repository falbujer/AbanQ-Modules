...
class oficial extends interna {
	...
	var posActualPuntoSubcuentaDieta:Number;
	...
	function comprobarSubcuentas() {
		return this.ctx.oficial_comprobarSubcuentas();
	}
}
//// OFICIAL /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
function interna_init()
{
	...
	this.iface.posActualPuntoSubcuentaDieta = -1;
	this.child("fdbIdSubcuenta").setFilter("codejercicio = '" + this.iface.ejercicioActual + "'");
	this.child("fdbIdSubcuentaDieta").setFilter("codejercicio = '" + this.iface.ejercicioActual + "'");
...
{
	if (!this.iface.comprobarSubcuentas()) {
		return false;
	}
	return true;
}
...
//////////////////////////////////////////////////////////////////
//// OFICIAL /////////////////////////////////////////////////////
function oficial_comprobarSubcuentas()
{
	var util:FLUtil = new FLUtil;
	var cursor:FLSqlCursor = this.cursor();
	...
	var codSubcuentaDieta:String = cursor.valueBuffer("codsubcuentadieta");
	var idSubcuentaDieta:Number = parseFloat(cursor.valueBuffer("idsubcuentadieta"));
	if (codSubcuentaDieta != util.sqlSelect("co_subcuentas","codsubcuenta","idsubcuenta = " + idSubcuentaDieta)) {
		MessageBox.warning(util.translate("scripts", "La subcuenta dieta establecida no es correcta"), MessageBox.Ok, MessageBox.NoButton);
		return false;
	}
	return true;
}
	...
	switch(fN) {
		...
		case "codsubcuentadieta": {
			...
			*/
			if (!this.iface.bloqueoSubcuenta) {
				this.iface.bloqueoSubcuenta = true;
				this.iface.posActualPuntoSubcuentaDieta = flcontppal.iface.pub_formatearCodSubcta(this, "fdbSubcuentaDieta", this.iface.longSubcuenta, this.iface.posActualPuntoSubcuentaDieta);
				this.iface.bloqueoSubcuenta = false;
			}
			break;
		}
...
function oficial_insertarHistorico()
{
	...
	curHistorico.setValueBuffer("cuotapatronalss", cursor.valueBuffer("cuotapatronalss"));
...
function oficial_cambiarBloqueo()
{
	...
	if(this.child("pbnBloqueo").on) {
		...
		this.child("fdbIrpf").setDisabled(true);
		this.child("fdbSegSocial").setDisabled(true);
		this.child("fdbCuotaPatronalSS").setDisabled(true);
		this.child("fdbFechaDesde").setDisabled(true);
		...
		this.child("fdbCuotaPatronalSS").setDisabled(false);
...
function oficial_calcularEdad()
{
	...
	var sNacimiento = fNacimiento.toString();
	var ano = parseInt(sNacimiento.left(4));
	var mes = parseFloat(sNacimiento.mid(5, 2));
	var dia = parseFloat(sNacimiento.mid(8, 2));
	
// 	var dNacimiento = new Date(Date.parse(fNacimiento.toString()));
	var hoy:Date = new Date();
	
	var edad:Number = hoy.getYear() - ano;
	var dNacimiento = new Date(Date.parse(hoy.toString()));
	dNacimiento.setDate(dia);
	dNacimiento.setMonth(mes);
	
	if (dNacimiento.getTime() > hoy.getTime()) {
		edad--;
	}
