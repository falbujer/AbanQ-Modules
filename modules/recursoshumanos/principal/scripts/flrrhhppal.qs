...
class interna {
	...
	function beforeCommit_rh_pagosdieta(curPD:FLSqlCursor):Boolean {
		return this.ctx.interna_beforeCommit_rh_pagosdieta(curPD);
	}
	function beforeCommit_rh_pagoshe(curPHE:FLSqlCursor):Boolean {
		return this.ctx.interna_beforeCommit_rh_pagoshe(curPHE);
	}
	...
	function afterCommit_rh_pagosdieta(curPD) {
		return this.ctx.interna_afterCommit_rh_pagosdieta(curPD);
	}
	function afterCommit_rh_pagoshe(curPHE) {
		return this.ctx.interna_afterCommit_rh_pagoshe(curPHE);
	}
...
class oficial extends interna {
	...
	function generarAsientoHorasExtra(curNomina:FLSqlCursor):Boolean {
		return this.ctx.oficial_generarAsientoHorasExtra(curNomina);
	}
	function generarAsientoPagosNomina(curPN:FLSqlCursor):Boolean {
		return this.ctx.oficial_generarAsientoPagosNomina(curPN);
	}
	function generarAsientoPagosDieta(curPD) {
		return this.ctx.oficial_generarAsientoPagosDieta(curPD);
	}
	...
	function generarPartidasCuotaPatronalSeguridadSocial(curNomina:FLSqlCursor, idAsiento:Number, valoresDefecto:Array):Boolean {
		return this.ctx.oficial_generarPartidasCuotaPatronalSeguridadSocial(curNomina, idAsiento, valoresDefecto);
	}
	function generarPartidasCuotaPatronal(curNomina:FLSqlCursor, idAsiento:Number, valoresDefecto:Array):Boolean {
		return this.ctx.oficial_generarPartidasCuotaPatronal(curNomina, idAsiento, valoresDefecto);
	}
	...
	function generarPartidasPagosDietaEmpleado(curPD, idAsiento, valoresDefecto) {
		return this.ctx.oficial_generarPartidasPagosDietaEmpleado(curPD, idAsiento, valoresDefecto);
	}
	function generarPartidasPagosDietaBanco(curPD, idAsiento, valoresDefecto) {
		return this.ctx.oficial_generarPartidasPagosDietaBanco(curPD, idAsiento, valoresDefecto);
	}
	...
	function datosPartidaHorasExtra(curPartida:FLSqlCursor, curPHE:FLSqlCursor):Array {
		return this.ctx.oficial_datosPartidaHorasExtra(curPartida, curPHE);
	}
	function datosPartidaPagosDieta(curPartida:FLSqlCursor, curNomina:FLSqlCursor):Array {
		return this.ctx.oficial_datosPartidaPagosDieta(curPartida, curNomina);
	}
	...
	function generarPartidasHorasExtra(curPHE:FLSqlCursor, idAsiento:Number, valoresDefecto:Array):Boolean {
		return this.ctx.oficial_generarPartidasHorasExtra(curPHE, idAsiento, valoresDefecto);
	}
	function generarPartidasHorasExtraEmpleado(curPHE:FLSqlCursor, idAsiento:Number, valoresDefecto:Array):Boolean {
		return this.ctx.oficial_generarPartidasHorasExtraEmpleado(curPHE, idAsiento, valoresDefecto);
	}
	...
	function hayCambiosNomina(curNomina) {
		return this.ctx.oficial_hayCambiosNomina(curNomina);
	}
	function crearSubcuenta(codSubcuenta, codEjercicio) {
		return this.ctx.oficial_crearSubcuenta(codSubcuenta, codEjercicio);
	}
}
//// OFICIAL /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
/*		if (this.iface.generarAsientoHorasExtra(curNomina) == false) {
			return false;
		}*/
	}
...
/** \C En el caso de que el módulo de contabilidad esté cargado y activado, genera o modifica el asiento contable correspondiente al pago de las dietas del empleado.
\end */
function interna_beforeCommit_rh_pagosdieta(curPD:FLSqlCursor):Boolean
{
	if (sys.isLoadedModule("flcontppal") && flfactppal.iface.pub_valorDefectoEmpresa("contintegrada")) {
		if (this.iface.generarAsientoPagosDieta(curPD) == false)
			return false;
	}
...
/** \C En el caso de que el módulo de contabilidad esté cargado y activado, genera o modifica el asiento contable correspondiente al pago de las horas extra del empleado.
\end */
function interna_beforeCommit_rh_pagoshe(curPHE:FLSqlCursor):Boolean
{
	if (sys.isLoadedModule("flcontppal") && flfactppal.iface.pub_valorDefectoEmpresa("contintegrada")) {
		if (this.iface.generarAsientoHorasExtra(curPHE) == false)
			return false;
	}
...
function interna_afterCommit_rh_pagosdieta(curPD)
{
	var util:FLUtil;
	...
	if (sys.isLoadedModule("flcontppal") && flfactppal.iface.pub_valorDefectoEmpresa("contintegrada")) {
		switch (curPD.modeAccess()) {
			case curPD.Del: {
				if (!this.iface.eliminarAsiento(curPD.valueBuffer("idasiento")))
					return false;
				break;
			}
			case curPD.Edit: {
...
function interna_afterCommit_rh_pagoshe(curPHE)
{
	var util:FLUtil;

	if (sys.isLoadedModule("flcontppal") && flfactppal.iface.pub_valorDefectoEmpresa("contintegrada")) {
		switch (curPHE.modeAccess()) {
			case curPHE.Del: {
				if (!this.iface.eliminarAsiento(curPHE.valueBuffer("idasiento")))
					return false;
				break;
			}
			case curPHE.Edit: {
				if (curPHE.valueBuffer("nogenerarasiento")) {
					var idAsientoAnterior:String = curPHE.valueBufferCopy("idasiento");
					if (idAsientoAnterior && idAsientoAnterior != "") {
						if (!this.iface.eliminarAsiento(idAsientoAnterior))
							return false;
					}
...
function oficial_generarAsientoNomina(curNomina:FLSqlCursor):Boolean
{
	...
	if (!this.iface.hayCambiosNomina(curNomina)) {
		return true;
	}
	...
	if (!this.iface.generarPartidasCuotaPatronalSeguridadSocial(curNomina, datosAsiento.idasiento, valoresDefecto))
		return false;

	if (!this.iface.generarPartidasCuotaPatronal(curNomina, datosAsiento.idasiento, valoresDefecto))
		return false;
	
	if (!curNomina.valueBuffer("pagoindependientedieta")) {
...
function oficial_hayCambiosNomina(curNomina)
{
	var aCampos = ["fecha", "codempleado", "sueldobruto", "irpf", "segsocial", "cuotapatronalss", "pagoindependientedieta"];
	for (var i = 0; i < aCampos.length; i++) {
		if (curNomina.valueBuffer(aCampos[i]) != curNomina.valueBufferCopy(aCampos[i])) {
			return true;
		}
	}
	return false;
...
function oficial_generarAsientoHorasExtra(curPHE)
{
	var util:FLUtil = new FLUtil;
	if (curPHE.valueBuffer("nogenerarasiento") || curPHE.valueBuffer("importe") == 0) {
		curPHE.setNull("idasiento");
		return true;
	}

	var datosAsiento:Array = [];
	var valoresDefecto:Array;
	valoresDefecto["codejercicio"] = util.sqlSelect("rh_nominas", "codejerciciohe", "codnomina = '" + curPHE.valueBuffer("codnomina") + "'");
	valoresDefecto["coddivisa"] = flfactppal.iface.pub_valorDefectoEmpresa("coddivisa");

	datosAsiento = this.iface.regenerarAsiento(curPHE, valoresDefecto);
	if (datosAsiento.error == true)
		return false;

	if (!this.iface.generarPartidasHorasExtra(curPHE, datosAsiento.idasiento, valoresDefecto))
		return false;

	if (!this.iface.generarPartidasHorasExtraEmpleado(curPHE, datosAsiento.idasiento, valoresDefecto))
		return false;

	curPHE.setValueBuffer("idasiento", datosAsiento.idasiento);
...
/** \U Genera o regenera el asiento correspondiente al pago de las dietas de empleado
@param	curPD: Cursor con los datos del pago de las dietas
@return	VERDADERO si no hay error. FALSO en otro caso
\end */
function oficial_generarAsientoPagosDieta(curPD)
{
	...
	var datosAsiento:Array = [];
	var valoresDefecto:Array;
	valoresDefecto["codejercicio"] = util.sqlSelect("rh_nominas", "codejercicio", "codnomina = '" + curPD.valueBuffer("codnomina") + "'");
	if (!valoresDefecto["codejercicio"] || valoresDefecto["codejercicio"] == "") {
		return false;
	}
	valoresDefecto["coddivisa"] = flfactppal.iface.pub_valorDefectoEmpresa("coddivisa");

	datosAsiento = this.iface.regenerarAsiento(curPD, valoresDefecto);
	if (datosAsiento.error == true)
		return false;

	if (!this.iface.generarPartidasPagosDietaEmpleado(curPD, datosAsiento.idasiento, valoresDefecto)) {
		return false;
	}

	if (!this.iface.generarPartidasPagosDietaBanco(curPD, datosAsiento.idasiento, valoresDefecto))
		return false;
...
/** \D Genera la parte del asiento de nomina correspondiente a la subcuenta de cuota patronal seguridad social
@param	curNomina: Cursor de la nómina
@param	idAsiento: Id del asiento asociado
@param	valoresDefecto: Array con los valores por defecto de ejercicio y divisa
@return	VERDADERO si no hay error, FALSO en otro caso
\end */
function oficial_generarPartidasCuotaPatronalSeguridadSocial(curNomina:FLSqlCursor, idAsiento:Number, valoresDefecto:Array):Boolean
{
	var util:FLUtil = new FLUtil();
	var cuotaPatronalSS:Number = parseFloat(curNomina.valueBuffer("cuotapatronalss"));
	if(cuotaPatronalSS == 0)
		return true;
	var ctaPatronalSeguridadSocial:Array = this.iface.datosCtaEspecial("NO_ASS", valoresDefecto.codejercicio);
	if (ctaPatronalSeguridadSocial.error != 0) {
		MessageBox.warning(util.translate("scripts", "No se ha encontrado una subcuenta de cuota patronal seguridad social para esta nómina.\nCompruebe que existe la cuenta especial NO_ASS asociada a una subcuenta."), MessageBox.Ok, MessageBox.NoButton);
			return false;
	}
	var haber:Number = 0;
	var haberME:Number = 0;
	haber = cuotaPatronalSS;
	haberME = 0;
	haber = util.roundFieldValue(haber, "co_partidas", "haber");
	haberME = util.roundFieldValue(haberME, "co_partidas", "haberme");
	...
	with (curPartida) {
		setModeAccess(curPartida.Insert);
		refreshBuffer();
		setValueBuffer("idsubcuenta", ctaPatronalSeguridadSocial.idsubcuenta);
		setValueBuffer("codsubcuenta", ctaPatronalSeguridadSocial.codsubcuenta);
		setValueBuffer("idasiento", idAsiento);
		setValueBuffer("debe", 0);
		setValueBuffer("haber", haber);
		setValueBuffer("debeME", 0);
...
function oficial_generarPartidasCuotaPatronal(curNomina:FLSqlCursor, idAsiento:Number, valoresDefecto:Array):Boolean
{
	var util:FLUtil = new FLUtil();
	var cuotaPatronalSS:Number = parseFloat(curNomina.valueBuffer("cuotapatronalss"));
	if(cuotaPatronalSS == 0)
		return true;
	var ctaPatronalSeguridadSocial:Array = this.iface.datosCtaEspecial("NO_PSS", valoresDefecto.codejercicio);
	if (ctaPatronalSeguridadSocial.error != 0) {
		MessageBox.warning(util.translate("scripts", "No se ha encontrado una subcuenta de cuota patronal seguridad social para esta nómina.\nCompruebe que existe la cuenta especial NO_PSS asociada a una subcuenta."), MessageBox.Ok, MessageBox.NoButton);
			return false;
	}
	var debe:Number = 0;
	var debeME:Number = 0;
	debe = cuotaPatronalSS;
	debeME = 0;
	debe = util.roundFieldValue(debe, "co_partidas", "debe");
	debeME = util.roundFieldValue(debeME, "co_partidas", "debeme");
...
/*	var util:FLUtil = new FLUtil();
	...
	var codEmpleado:String = util.sqlSelect("rh_nominas", "codempleado", "codnomina = '" + curNomina.valueBuffer("codnomina") + "'");
	if (!codEmpleado || codEmpleado == "") {
		return false;
	}

	var ctaDebe:Array = [];
	ctaDebe.codsubcuenta = util.sqlSelect("rh_empleados", "codsubcuentadieta", "codempleado = '" + codEmpleado + "'");
	if (!ctaDebe.codsubcuenta || ctaDebe.codsubcuenta == "") {
		MessageBox.warning(util.translate("scripts", "El empleado %1 no tiene ninguna subcuenta contable dietas asociada.\nAntes de generar el pago de la nómina debe asociar la subcuenta al empleado").arg(codEmpleado), MessageBox.Ok, MessageBox.NoButton);
		return false;
	}
	...
	if (!ctaDebe.idsubcuenta) {
		var res = MessageBox.warning(util.translate("scripts", "No tiene definida la subcuenta %1 en el ejercicio %2.\n¿Desea crearla?").arg(ctaDebe.codsubcuenta).arg(valoresDefecto.codejercicio), MessageBox.Yes, MessageBox.No);
		if (res != MessageBox.Yes) {
			return false;
		} else {
			if (!this.iface.crearSubcuenta(ctaDebe.codsubcuenta, valoresDefecto.codejercicio)) {
				return false;
			}
			ctaDebe.idsubcuenta = util.sqlSelect("co_subcuentas", "idsubcuenta", "codsubcuenta = '" + ctaDebe.codsubcuenta + "' AND codejercicio = '" + valoresDefecto.codejercicio + "'");
		}
...
function oficial_generarPartidasNominaEmpleado(curNomina:FLSqlCursor, idAsiento:Number, valoresDefecto:Array):Boolean
{
	...
	ctaHaber.idsubcuenta = util.sqlSelect("co_subcuentas", "idsubcuenta", "codsubcuenta = '" + ctaHaber.codsubcuenta + "' AND codejercicio = '" + valoresDefecto.codejercicio + "'");
	if (!ctaHaber.idsubcuenta) {
		var res = MessageBox.warning(util.translate("scripts", "No tiene definida la subcuenta %1 en el ejercicio %2.\n¿Desea crearla?").arg(ctaHaber.codsubcuenta).arg(valoresDefecto.codejercicio), MessageBox.Yes, MessageBox.No);
		if (res != MessageBox.Yes) {
			return false;
		} else {
			if (!this.iface.crearSubcuenta(ctaHaber.codsubcuenta, valoresDefecto.codejercicio)) {
				return false;
			}
			ctaHaber.idsubcuenta = util.sqlSelect("co_subcuentas", "idsubcuenta", "codsubcuenta = '" + ctaHaber.codsubcuenta + "' AND codejercicio = '" + valoresDefecto.codejercicio + "'");
		}
...
/** \D Genera la partida correspondiente al empleado
@param	curPD: Cursor del pago de las dietas
@param	valoresDefecto: Array de valores por defecto (ejercicio, divisa, etc.)
@param	datosAsiento: Array con los datos del asiento
@return	true si la generación es correcta, false en caso contrario
\end */
function oficial_generarPartidasPagosDietaEmpleado(curPD, idAsiento, valoresDefecto)
{
	var util:FLUtil = new FLUtil();
	var importe:Number = parseFloat(curPD.valueBuffer("importe"));
	if (importe == 0) {
		return true;
	}
	var codEmpleado:String = util.sqlSelect("rh_nominas", "codempleado", "codnomina = '" + curPD.valueBuffer("codnomina") + "'");
	if (!codEmpleado || codEmpleado == "") {
		return false;
	}
	...
	this.iface.datosPartidaPagosDieta(curPartida, curPD)	
...
/** \D Genera la partida correspondiente a la empresa del asiento de pago de las dietas del empleado
@param	curPD: Cursor del pago de las dietas
@param	valoresDefecto: Array de valores por defecto (ejercicio, divisa, etc.)
@param	datosAsiento: Array con los datos del asiento
@return	true si la generación es correcta, false en caso contrario
\end */
function oficial_generarPartidasPagosDietaBanco(curPD, idAsiento, valoresDefecto)
{
	var util:FLUtil = new FLUtil();
	var importe:Number = parseFloat(curPD.valueBuffer("importe"));
	if (importe == 0) {
	...
	if (!curPD.valueBuffer("codcuenta") || curPD.valueBuffer("codcuenta") == "") {
		MessageBox.warning(util.translate("scripts", "Debe establecer la cuenta de pago de la empresa"), MessageBox.Ok, MessageBox.NoButton);
		return false;
	}
	
	var ctaHaber:Array = [];
	ctaHaber.codsubcuenta = util.sqlSelect("cuentasbanco","codsubcuenta","codcuenta = '" + curPD.valueBuffer("codcuenta") + "'");

	if (!ctaHaber.codsubcuenta || ctaHaber.codsubcuenta == "") {
		MessageBox.warning(util.translate("scripts", "Debe establecer la subcuenta en la cuenta bancaria de la empresa"), MessageBox.Ok, MessageBox.NoButton);
		return false;
	}
...
function oficial_generarPartidasEmpresa(curPN:FLSqlCursor, idAsiento:Number, valoresDefecto:Array):Boolean
{
	...
	if (!curPN.valueBuffer("codcuenta") || curPN.valueBuffer("codcuenta") == "") {
		MessageBox.warning(util.translate("scripts", "Debe establecer la cuenta de la empresa"), MessageBox.Ok, MessageBox.NoButton);
		return false;
	}
	...
	if (!ctaHaber.codsubcuenta || ctaHaber.codsubcuenta == "") {
		MessageBox.warning(util.translate("scripts", "Debe establecer la subcuenta en la cuenta de la empresa"), MessageBox.Ok, MessageBox.NoButton);
		return false;
	}
...
function oficial_generarPartidasHorasExtraEmpleado(curPHE:FLSqlCursor, idAsiento:Number, valoresDefecto:Array):Boolean
{
	var util:FLUtil = new FLUtil();
	var totalHE:Number = parseFloat(curPHE.valueBuffer("importe"));
	if (totalHE == 0) {
		return true;
	}
	
	var qryEmpleado:FLSqlQuery = new FLSqlQuery();
	qryEmpleado.setTablesList("rh_nominas,rh_empleados");
	qryEmpleado.setSelect("e.codempleado, e.nombre, e.apellidos, e.codsubcuenta");
	qryEmpleado.setFrom("rh_nominas n INNER JOIN rh_empleados e ON n.codempleado = n.codempleado");
	qryEmpleado.setWhere("n.codnomina = '" + curPHE.valueBuffer("codnomina") + "'");

	if (!qryEmpleado.exec()) {
		return false;
	}

	var nombreEmpleado, apellidosEmpleado, subcuentaEmpleado, codEmpleado;
	if (qryEmpleado.first()) {
		nombreEmpleado = qryEmpleado.value("e.nombre");
		apellidosEmpleado = qryEmpleado.value("e.apellidos");
		codEmpleado = qryEmpleado.value("e.codempleado");
		subcuentaEmpleado = qryEmpleado.value("e.codsubcuenta");
	}

	var ctaHaber:Array = [];
	ctaHaber.codsubcuenta = subcuentaEmpleado;
	if (!ctaHaber.codsubcuenta || ctaHaber.codsubcuenta == "") {
		MessageBox.warning(util.translate("scripts", "El empleado %1 - %2 %3 no tiene ninguna subcuenta contable asociada.\nAntes de generar la nómina debe asociar la subcuenta al empleado").arg(codEmpleado).arg(nombreEmpleado).arg(apellidosEmpleado), MessageBox.Ok, MessageBox.NoButton);
		return false;
	}
	ctaHaber.idsubcuenta = util.sqlSelect("co_subcuentas", "idsubcuenta", "codsubcuenta = '" + ctaHaber.codsubcuenta + "' AND codejercicio = '" + valoresDefecto.codejercicio + "'");
	if (!ctaHaber.idsubcuenta) {
	...
	this.iface.datosPartidaHorasExtra(curPartida, curPHE);
...
function oficial_generarPartidasHorasExtra(curPHE:FLSqlCursor, idAsiento:Number, valoresDefecto:Array):Boolean
{
	var util:FLUtil = new FLUtil();
	var totalHE:Number = parseFloat(curPHE.valueBuffer("importe"));
	if (totalHE == 0) {
	...
	var ctaHorasExtra:Array = this.iface.datosCtaEspecial("NO_HE",valoresDefecto.codejercicio);
	if (ctaHorasExtra.error != 0) {
		MessageBox.warning(util.translate("scripts", "No se ha encontrado una subcuenta de horas extra para esta nómina.\nCompruebe que existe la cuenta especial NO_HE asociada a una subcuenta."), MessageBox.Ok, MessageBox.NoButton);
		return false;
	}
	var debe:Number = 0;
	var debeME:Number = 0;
	...
	with (curPartida) {
			setModeAccess(curPartida.Insert);
			refreshBuffer();
			setValueBuffer("idsubcuenta", ctaHorasExtra.idsubcuenta);
			setValueBuffer("codsubcuenta", ctaHorasExtra.codsubcuenta);
			setValueBuffer("idasiento", idAsiento);
			setValueBuffer("debe", debe);
			setValueBuffer("haber", 0);
			setValueBuffer("debeME", debeME);
	...
	if (cur.isNull("idasiento")) {
		...
		switch (cur.table()) {
			...
			case "rh_pagoshe": {
				...
				concepto = "Pago horas extra empleado " + codEmpleado + " - " + nombre + " " + apellidos + " de " + fechaNomina;
				break;
			}
			case "rh_pagosdieta": {
				var codEmpleado:String = util.sqlSelect("rh_nominas","codempleado","codnomina = '" + cur.valueBuffer("codnomina") + "'");
				...
				concepto = "Pago dietas empleado " + codEmpleado + " - " + nombre + " " + apellidos + " de " + fechaNomina;
				break;
			}
		}
...
function oficial_datosPartidaHorasExtra(curPartida:FLSqlCursor, curPHE:FLSqlCursor) 
{
	var util:FLUtil = new FLUtil();

	var codEmpleado:String = util.sqlSelect("rh_nominas","codempleado","codnomina = '" + curPHE.valueBuffer("codnomina") + "'");
	if (!codEmpleado || codEmpleado == "")
		return false;
	...
	var fechaPago:String = util.dateAMDtoDMA(curPHE.valueBuffer("fecha"));
	fechaPago = fechaPago.right(7);
	
	curPartida.setValueBuffer("concepto", util.translate("scripts", "Horas extra empleado %1 - %2 %3 de %4 ").arg(codEmpleado).arg(nombre).arg(apellidos).arg(fechaPago));
}
...
/** \D Establece los datos opcionales de una partida de pago de dietas.
Se ponen datos de concepto
@param	curPartida: Cursor sobre la partida
@param	curPD: Cursor sobre el pago de las dietas
*/
function oficial_datosPartidaPagosDieta(curPartida:FLSqlCursor, curPD:FLSqlCursor) 
{
	var util:FLUtil = new FLUtil();

	var codEmpleado = util.sqlSelect("rh_nominas","codempleado","codnomina = '" + curPD.valueBuffer("codnomina") + "'");
	if (!codEmpleado || codEmpleado == "")
		return false;

	var nombre = util.sqlSelect("rh_empleados","nombre","codempleado = '" + codEmpleado + "'");
	if (!nombre || nombre == "")
		return false;

	var apellidos = util.sqlSelect("rh_empleados","apellidos","codempleado = '" + codEmpleado + "'");
	if (!apellidos || apellidos == "")
		return false;

	var fechaPago = util.dateAMDtoDMA(curPD.valueBuffer("fecha"));
	
	curPartida.setValueBuffer("concepto", util.translate("scripts", "Pago dietas empleado %1 - %2 %3 de %4 ").arg(codEmpleado).arg(nombre).arg(apellidos).arg(fechaPago));
}
...
function oficial_crearSubcuenta(codSubcuenta, codEjercicio)
{
	var util = new FLUtil();
	var curSubcuentaOrigen = new FLSqlCursor("co_subcuentas");
	curSubcuentaOrigen.select("codsubcuenta = '" + codSubcuenta + "'");
	if (!curSubcuentaOrigen.first()) {
		return false;
	}
	curSubcuentaOrigen.setModeAccess(curSubcuentaOrigen.Edit);
	curSubcuentaOrigen.refreshBuffer();

	if (!formco_subcuentas.iface.pub_copiarSubcuentaEjercicio(curSubcuentaOrigen, codEjercicio)) {
		return false;
	}
	return true;
}
