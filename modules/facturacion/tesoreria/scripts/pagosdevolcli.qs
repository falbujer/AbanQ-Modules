...
class oficial extends interna {
	...
	*/
	...
	function validaFechaPago() {
		return this.ctx.oficial_validaFechaPago();
	}
}
//// OFICIAL /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
function interna_init()
{
	...
	this.iface.contabActivada = sys.isLoadedModule("flcontppal") && flfactppal.iface.pub_valorDefectoEmpresa("contintegrada");
			...
			if (last) {
				if (curPagosDevol.valueBuffer("tipo") == "Pago") {
					if (!this.iface.comprobarRemesaPagada(curPagosDevol.valueBuffer("idremesa"))) {
						sys.AQTimer.singleShot(0, formRecordpagosdevolcli.close);
// 						this.close();
						return;
					}
...
*/
	...
	if (ejercicioFactura && this.iface.ejercicioActual != ejercicioFactura) {
...
function oficial_validaFechaPago()
{
	var _i = this.iface;
	var cursor = this.cursor();
	var util = new FLUtil();
