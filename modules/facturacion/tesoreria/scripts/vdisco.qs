...
class oficial extends interna {
	...
	function individualesOpcionales(qryRecibos) {
		return this.ctx.oficial_individualesOpcionales(qryRecibos);
	}
	...
	function chkAgruparCliente_toggled(agrupar) {
		return this.ctx.oficial_chkAgruparCliente_toggled(agrupar);
	}
...
function interna_init()
{
	...
	connect(this.child("chkAgruparCliente"), "toggled(bool)", this, "iface.chkAgruparCliente_toggled")
}
...
function interna_acceptedForm()
{
	...
	if (this.child("chkAgruparCliente").checked) {
		...
		with (qryRecibos) {
			setTablesList("reciboscli");
			setSelect("importe, codigo, codcliente, nombrecliente, codcuenta, ctaentidad, ctaagencia, dc, cuenta, idfactura");
			setFrom("reciboscli");
...
debug("va");
		if (this.child("chkIncluirLF").checked && !this.child("chkAgruparCliente").checked) {
debug("va1");
			file.write(this.iface.individualesOpcionales(qryRecibos));
		}
	}
...
@return Texto con los dato para ser volcados a fichero
\end */
function oficial_individualObligatorio(qryRecibos)
{
	var util:FLUtil = new FLUtil();
	var reg:String = "5680";
	var cifnif:String = flfactppal.iface.pub_valorDefectoEmpresa("cifnif");
...
function oficial_individualesOpcionales(qryRecibos)
{
	var util = new FLUtil;
	var idFactura = qryRecibos.value("idfactura");
	if (!idFactura) {
		return "";
	}
	var qryLineas = new FLSqlQuery();
	qryLineas.setTablesList("lineasfacturascli");
	qryLineas.setSelect("descripcion");
	qryLineas.setFrom("lineasfacturascli");
	qryLineas.setWhere("idfactura = " + idFactura);
	qryLineas.setForwardOnly(true);
	if (!qryLineas.exec()) {
debug("falló la consulta " + qryLineas.sql());
		return "";
	}
	var concepto = "";
	while (qryLineas.next()) {
		concepto += concepto == "" ? "" : ". ";
		concepto += qryLineas.value("descripcion");
	}
debug("Concepto = " + concepto);
	if (concepto.length > 600) {
		concepto = concepto.left(600);
	}
	var lon = concepto.length;
	var numReg = Math.ceil(lon / 120);
	var conceptoReg;
	var regs = "";
	var cifnif:String = flfactppal.iface.pub_valorDefectoEmpresa("cifnif");
	var codCliente:String = qryRecibos.value("codcliente").right(6);
	...
	var codcuenta = this.cursor().valueBuffer("codcuenta");
	var sufijo = util.sqlSelect("cuentasbanco", "sufijo", "codcuenta = '" + codcuenta + "'");
	sufijo = flfactppal.iface.pub_cerosIzquierda(sufijo, 3);
	
	for (var i = 0; i  < numReg; i++) {
		conceptoReg = concepto.mid(i * 120, 120);
		conceptoReg = flfactppal.iface.pub_espaciosDerecha(conceptoReg, 120);
debug("conceptoReg = " + conceptoReg);
		regs += "568" + (i + 1).toString();
		regs += flfactppal.iface.pub_cerosIzquierda(cifnif + sufijo, 12).right(12);
		regs += flfactppal.iface.pub_espaciosDerecha(ref, 12);
		regs += conceptoReg;
		regs += flfactppal.iface.pub_espaciosDerecha("", 14);
		regs += "\r\n";
	}
	return regs;
}
...
function oficial_chkAgruparCliente_toggled(agrupar)
{
	if (agrupar) {
		this.child("chkIncluirLF").checked = false;
		this.child("chkIncluirLF").enabled = false;
	} else {
		this.child("chkIncluirLF").enabled = true;
	}
}
//// OFICIAL /////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
