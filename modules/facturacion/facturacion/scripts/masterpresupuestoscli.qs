...
class oficial extends interna {
	...
	function tbnProforma_clicked() {
		return this.ctx.oficial_tbnProforma_clicked();
	}
	function dameParamInforme(idPresupuesto) {
		return this.ctx.oficial_dameParamInforme(idPresupuesto);
	}
	...
	function metGenerarPedido(curId) {
		return this.ctx.oficial_metGenerarPedido(curId);
	}
...
class ifaceCtx extends head {
	...
	function pub_metGenerarPedido(cursor) {
		return this.metGenerarPedido(cursor);
	}
}
...
{
	var _i = this.iface;
	_i.pbnGPedido = this.child("pbnGenerarPedido");
	_i.tdbRecords= this.child("tableDBRecords");
	_i.tbnImprimir = this.child("toolButtonPrint");
	_i.tbnDuplicarPres = this.child("tbnDuplicarPres");
...
\end */
function oficial_imprimir(codPresupuesto)
{
	var util = new FLUtil;
	if (sys.isLoadedModule("flfactinfo")) {
		flfactinfo.iface.pub_activaProforma(false);
		var idPresupuesto, codigo;
		if (codPresupuesto) {
		...
		var oParam = this.iface.dameParamInforme(idPresupuesto);
		if (!oParam) {
			return;
		}
...
function oficial_tbnProforma_clicked()
{
	var _i = this.iface;
	var cursor = this.cursor();
	...
	flfactinfo.iface.pub_activaProforma(true);
	
		
	var oParam = _i.dameParamInforme(idPresupuesto);
	if (!oParam) {
		return;
	}
	var curImprimir = new FLSqlCursor("i_presupuestoscli");
	curImprimir.setModeAccess(curImprimir.Insert);
	curImprimir.refreshBuffer();
	curImprimir.setValueBuffer("descripcion", "temp");
	curImprimir.setValueBuffer("d_presupuestoscli_codigo", codigo);
...
function oficial_dameParamInforme(idPresupuesto)
{
	var util = new FLUtil;
	var oParam = flfactinfo.iface.pub_dameParamInforme();
	oParam.nombreInforme = "i_presupuestoscli";
	return oParam;
}
...
function oficial_metGenerarPedido(curId)
{
	var _i = this.iface;
	var curPresupuesto;
	if (typeof(curId) == "String" || typeof(curId) == "Number") {
		curPresupuesto = new FLSqlCursor("presupuestoscli");
		curPresupuesto.select("idpresupuesto = " + curId);
		if (!curPresupuesto.first()) {
			return false;
		}
	} else {
		curPresupuesto = curId;
	}
	return _i.generarPedido(curPresupuesto);
}
/** \D
