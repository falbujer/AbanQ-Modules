...
class oficial extends interna {
	...
	var aLineasRect_; /// Lista con los valores
	...
	function copiarLineasRec(idFacturaOriginal, aLineas, factor) {
		return this.ctx.oficial_copiarLineasRec(idFacturaOriginal, aLineas, factor);
	}
	function copiarCampoLineaRec(nombreCampo, curLineaOriginal, factor, campoInformado) {
		return this.ctx.oficial_copiarCampoLineaRec(nombreCampo, curLineaOriginal, factor, campoInformado);
	}
	...
	function seleccionLineasRec() {
		return this.ctx.oficial_seleccionLineasRec();
	}
...
function oficial_inicializarControles()
{
	...
	if (this.child("tdbRecibosCli")) {
		this.child("tdbRecibosCli").setEditOnly(true);
		this.child("tdbRecibosCli").cursor().setAction("recibosfacturacli");
	}
...
function oficial_mostrarOpcionesFacturaRec(idFactura)
{
	var util = new FLUtil;
	var opciones = [util.translate("scripts", "Copiar líneas de la factura"), util.translate("scripts", "Copiar con cantidad negativa"), util.translate("scripts", "No copiar líneas")];
	var opcion:Number = flfactppal.iface.pub_elegirOpcion(opciones);
	switch (opcion) {
		case 0:
		case 1: {
			var aDatosFR = new Object;
			aDatosFR.tabla = "lineasfacturascli";
			aDatosFR.signo = opcion == 0 ? "+" : "-";
			aDatosFR.idfactura = idFactura;
			flfacturac.iface.pub_ponDatosFacturaRec(aDatosFR);
			var valores = this.iface.seleccionLineasRec();
			if (!valores) {
				return false;
			}
			var factor = aDatosFR.signo == "+" ? 1 : -1;
			if (!this.iface.copiarLineasRec(idFactura, valores, factor)) {
				return false;
			}
			break;
		}
...
function oficial_copiarLineasRec(idFacturaOriginal, aLineas, factor)
{
	var util:FLUtil = new FLUtil;
	var cursor:FLSqlCursor = this.cursor();
	...
	for (var i = 0; i < aLineas.length; i++) {
		...
		this.iface.curLineaRectificacion_.setValueBuffer("cantidad", aLineas[i]["cantidad"]);
...
// 	var miFiltro = filtro && filtro != "" ? " AND " + filtro : "";
// 	curLineaOriginal.select("idfactura = " + idFacturaOriginal + miFiltro + " ORDER BY idlinea");
...
function oficial_copiarCampoLineaRec(nombreCampo, curLineaOriginal, factor, campoInformado)
{
	...
	switch (nombreCampo) {
		...
		case "pvpsindto": {
			if (!campoInformado["pvpunitario"]) {
				if (!this.iface.copiarCampoLineaRec("pvpunitario", curLineaOriginal, factor, campoInformado)) {
					return false;
				}
			}
			this.iface.curLineaRectificacion_.setValueBuffer("pvpsindto", formRecordlineaspedidoscli.iface.pub_commonCalculateField("pvpsindto", this.iface.curLineaRectificacion_));
			break;
		}
		case "pvptotal": {
			if (!campoInformado["pvpsindto"]) {
				if (!this.iface.copiarCampoLineaRec("pvpsindto", curLineaOriginal, factor, campoInformado)) {
					return false;
				}
			}
			if (!campoInformado["dtolineal"]) {
				if (!this.iface.copiarCampoLineaRec("dtolineal", curLineaOriginal, factor, campoInformado)) {
					return false;
				}
			}
			if (!campoInformado["dtopor"]) {
				if (!this.iface.copiarCampoLineaRec("dtopor", curLineaOriginal, factor, campoInformado)) {
					return false;
				}
			}
			this.iface.curLineaRectificacion_.setValueBuffer("pvptotal", formRecordlineaspedidoscli.iface.pub_commonCalculateField("pvptotal", this.iface.curLineaRectificacion_));
			break;
		}
		case "idsubcuenta": {
...
function oficial_seleccionLineasRec()
{
	var valores = "";
	
	var curLineasRec = new FLSqlCursor("lineasfacturascli");
// 	curLineasRec.setMainFilter("idfactura = " + idFacturaOriginal);
	var f= new FLFormSearchDB(curLineasRec, "seleccionlineasrec");
	
	f.setMainWidget();
	f.exec();
	
	if (f.accepted()) {
 		valores = this.iface.aLineasRect_;
	} else {
		valores = false;
	}
...
function oficial_ponSeleccionValores(valores)
{
	this.iface.aLineasRect_ = valores;
}
...
function oficial_actualizarLineasIva(curFactura:FLSqlCursor):Boolean
{
	...
	var hayLineas = qryLineasFactura.size() > 0;
...
/// Quitado porque en staco hay facturas con líneas positiva y negativa que no generan IVA 	if (totalNeto != 0) {
	if (hayLineas) {
