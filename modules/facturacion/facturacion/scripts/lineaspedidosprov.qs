...
function oficial_commonCalculateField(fN:String, cursor:FLSqlCursor):String
{
	...
	switch (fN) {
		...
		case "iva":{
			valor = flfacturac.iface.pub_campoImpuesto("iva", cursor.valueBuffer("codimpuesto"), datosTP["fecha"], "prov");
			if (isNaN(valor)) {
				valor = "";
			}
			break;
...
// 			valor = flfactppal.iface.pub_valorQuery("descuentosproveedores,descuentos", "SUM(d.dto)", "descuentosproveedores dc INNER JOIN descuentos d ON dc.coddescuento = d.coddescuento", "dc.codproveedor = '" + codProveedor + "'");
		...
		case "recargo":{
			...
			if (aplicarRecEq == true) {
				valor = flfacturac.iface.pub_campoImpuesto("recargo", cursor.valueBuffer("codimpuesto"), datosTP["fecha"], "prov");
			} else {
				valor = "";
			}
			if (isNaN(valor)) {
