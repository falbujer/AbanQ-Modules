...
function oficial_imprimir(codAlbaran:String)
{
	if (sys.isLoadedModule("flfactinfo")) {
		...
		if (codAlbaran) {
			codigo = codAlbaran;
			idAlbaran = util.sqlSelect("albaranesprov","idalbaran","codigo = '" + codigo + "'");
		} else {
...
function oficial_dameParamInforme(idAlbaran)
{
	...
	oParam.nombreInforme = "i_albaranesprov";
	return oParam;
}
...
function oficial_generarFactura(where:String, curAlbaran:FLSqlCursor, datosAgrupacion:Array):Number
{
	...
	if (this.iface.curFactura.first()) {
		...
		if (!flfacturac.iface.pub_actualizarArticulosProv(this.iface.curFactura)) {
			return false;
		}
		if (!this.iface.curFactura.commitBuffer()) {
			return false;
