...
function oficial_generarFactura(where: String, curAlbaran: FLSqlCursor, datosAgrupacion: Array): Number {
  ...
  while (curAlbaranes.next())
  {
    ...
    if (!curAlbaranes.valueBuffer("ptefactura")) {
      ...
      MessageBox.warning(sys.translate("Error al facturar el albarán %1. Este albarán ya está facturado en la factura %2").arg(curAlbaranes.valueBuffer("codigo")).arg(codFacturaAlb), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton, "AbanQ");
      return false;
    }
