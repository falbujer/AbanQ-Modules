...
class oficial extends interna {
	...
	var posActualPuntoSubIVASopSPUE;
	var posActualPuntoSubIVARepSPUE;
	...
	function comprobarSubcuentaIVA(codSubcuenta:String, idCuentaEsp) {
		return this.ctx.oficial_comprobarSubcuentaIVA(codSubcuenta, idCuentaEsp);
	}
	function crearCuentaEsp(idCuentaEsp:String) {
...
function interna_init()
{
	...
	this.child("fdbIdSubcuentaIVARepSPUE").setFilter("codejercicio = '" + ejercicioActual + "'");
	this.child("fdbIdSubcuentaIVASopSPUE").setFilter("codejercicio = '" + ejercicioActual + "'");
...
\end */
	if (!sys.isLoadedModule("flcontppal"))
		...
		this.iface.posActualPuntoSubIVARepSPUE = -1;
		this.iface.posActualPuntoSubIVASopSPUE = -1;
...
debug("interna_validateForm");
	...
	if (!this.iface.comprobarSubcuentaIVA(cursor.valueBuffer("codsubcuentaivarepspue"), "IVARSE")) {
		return false;
	}
	if (!this.iface.comprobarSubcuentaIVA(cursor.valueBuffer("codsubcuentaivasopspue"), "IVASSE")) {
		return false;
	}
	return true;
}
...
//////////////////////////////////////////////////////////////////
//// OFICIAL /////////////////////////////////////////////////////
function oficial_comprobarSubcuentaIVA(codSubcuenta:String, idCuentaEsp:String)
{
...
function oficial_crearCuentaEsp(idCuentaEsp:String)
{
debug("oficial_crearCuentaEsp " + idCuentaEsp);
	...
	switch (idCuentaEsp) {
		...
		case "IVARSRE": {
			descripcion = util.translate("scripts", "I.V.A. Repercutido en recepción de servicios intracomunitarios");
			break;
		}
		case "IVARSSE": {
			descripcion = util.translate("scripts", "I.V.A. Soportado en recepción de servicios intracomunitarios");
			break;
		}
	}
	var curCuentaEsp= new FLSqlCursor("co_cuentasesp");
	with (curCuentaEsp) {
...
function oficial_bufferChanged(fN)
{
	...
	switch(fN) {
		...
		case "codsubcuentaivarepspue": {
			if (!this.iface.bloqueoSubcuenta) {
				this.iface.bloqueoSubcuenta = true;
				this.iface.posActualPuntoSubIVARepSPUE= flcontppal.iface.pub_formatearCodSubcta(this, "fdbCodSubcuentaIVARepSPUE", this.iface.longSubcuenta, this.iface.posActualPuntoSubIVARepSPUE);
				this.iface.bloqueoSubcuenta = false;
			}
			break;
		}
		case "codsubcuentaivasopspue": {
			if (!this.iface.bloqueoSubcuenta) {
				this.iface.bloqueoSubcuenta = true;
				this.iface.posActualPuntoSubIVASopSPUE= flcontppal.iface.pub_formatearCodSubcta(this, "fdbCodSubcuentaIVASopSPUE", this.iface.longSubcuenta, this.iface.posActualPuntoSubIVASopSPUE);
				this.iface.bloqueoSubcuenta = false;
			}
			break;
		}
