...
function oficial_rellenarIntervalos()
{
	...
	var intervalos:Array =
		...
		["000011", util.translate("scripts", "Mañana"), util.translate("scripts", "Muestra todos los informes con fecha de mañana"), "hoy = new Date();\n" +
			"intervalo.desde = util.addDays(hoy,+1);\n" + 
			"intervalo.hasta = util.addDays(hoy,+1);\n" + 
			"return intervalo;\n"],
		["000012", util.translate("scripts", "Semana que viene"), util.translate("scripts", "Muestra todos los informes de la semana que viene"), "desde = new Date();\n" + 
			"dias = desde.getDay()-1;\n" +
			"dias = 7-dias;\n" +
			"intervalo.desde = util.addDays(desde, dias);\n" +
			"intervalo.hasta = util.addDays(intervalo.desde,6);\n" +
			"return intervalo;\n"],
		["000013", util.translate("scripts", "Mes que viene"), util.translate("scripts", "Muestra todos los informes del mes que viene"), "primerdia = new Date();\n" +
			"desde = primerdia.setDate(1);\n" +
			"desde = util.addMonths(desde, 1);\n" +
			"intervalo.desde = desde;\n" +
			"ultimodia = new Date();\n" +
			"ultimodia = util.addMonths(desde, 1)\n" +
			"hasta = ultimodia.setDate(1);\n" +
			"hasta = util.addDays(hasta,-1);\n" +
			"intervalo.hasta = hasta;\n" +
			"return intervalo;\n"],
		["000014", util.translate("scripts", "Año que viene"), util.translate("scripts", "Muestra todos los informes del año que viene"), "desde = new Date();\n" +
			"anio = desde.getYear() + 1;\n" + 
			"desde = desde.setDate(1);\n" + 
			"desde = desde.setMonth(1);\n" + 
			"desde = desde.setYear(anio);\n" + 
	...
	for (var i:Number = 0; i < intervalos.length; i++) {
		...
		if(AQUtil.sqlSelect("intervalos","codigo","codigo = '" + intervalos[i][0] + "'"))
			continue;
