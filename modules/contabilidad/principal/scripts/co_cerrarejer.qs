...
class oficial extends interna {
	...
	function comprobarPartidasHuerfanas():Boolean {
		return this.ctx.oficial_comprobarPartidasHuerfanas();
	}
}
//// OFICIAL /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
function oficial_validarCierre():Boolean
{
	...
	if (this.child("ckbAsientoApertura").checked == true) {
		if (this.child("ledEjercicio").text == "") {
			MessageBox.warning(util.translate("scripts", "Debe seleccionar el ejercicio sobre el que se creará el asiento de apertura"), MessageBox.Ok, MessageBox.NoButton);
			return false;
		}

		var numAsientoApertura:Number = util.sqlSelect("ejercicios,co_asientos", "co_asientos.numero", "co_asientos.idasiento=ejercicios.idasientoapertura AND ejercicios.codejercicio = '" + this.child("ledEjercicio").text + "'");
		if (numAsientoApertura) {
			MessageBox.warning(util.translate("scripts", "El ejercicio seleccionado tiene ya un asiento de apertura, borre el asiento o cambie el ejercicio.\nEl asiento de apertura para el ejercicio seleccionado es el número ") + numAsientoApertura, MessageBox.Ok, MessageBox.NoButton);
			return false;
		}
	}

	if (!this.iface.comprobarPartidasHuerfanas()) {
		MessageBox.critical(util.translate("scripts", "Existen partidas sin asiento asociado para el ejercicio actual. Esto no debería ocurrir. Contacte con InfoSiAL para solucionar el problema."), MessageBox.Ok, MessageBox.NoButton);
		return false;
	}
	debug("VC OK");
	return true;
...
/** \D Comprueba si hay partidas sin asiento para evitar cierres erróneos.
\end */
function oficial_comprobarPartidasHuerfanas():Boolean
{
	var util:FLUtil = new FLUtil;
	var cursor:FLSqlCursor = this.cursor();
	var codEjercicio:String = cursor.valueBuffer("codejercicio");
debug("codEjercicio " + codEjercicio);
	if (util.sqlSelect("co_subcuentas s INNER JOIN co_partidas p ON s.idsubcuenta = p.idsubcuenta LEFT OUTER JOIN co_asientos a ON p.idasiento = a.idasiento", "p.idpartida", "s.codejercicio = '" + codEjercicio + "' AND a.idasiento IS NULL", "co_subcuentas,co_partidas,co_asientos")) {
		return false;
	}
	return true;
}
