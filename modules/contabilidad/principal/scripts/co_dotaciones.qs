...
function interna_validateForm():Boolean
{
	...
	if(!ultimaFechaFin) {
		...
		ultimaFechaFin = curR.isNull("fechainicio") ?  curR.valueBuffer("fecha") : curR.valueBuffer("fechainicio");
	}
...
function oficial_commonCalculateField(fN:String, cursor:FLSqlCursor):String
{
	...
	switch(fN) {
		case "importe": {
			...
			res = AQUtil.roundFieldValue(res, "co_dotaciones", "importe");
			break;
		}
		case "porcentaje": {
			var valorAmortizable:Number;
			...
			res = AQUtil.roundFieldValue(res, "co_dotaciones", "porcentaje");
			break;
		}
		case "acumulado": {
			...
			res = AQUtil.roundFieldValue(res, "co_dotaciones", "acumulado");
			break;
		}
		case "resto": {
			var valorAmortizable:Number;
			...
			res = AQUtil.roundFieldValue(res, "co_dotaciones", "resto");
			break;
		}
		case "fechainicio": {
			...
			if (!fechaInicio) {
				if (cursor.cursorRelation()) {
					var curR = cursor.cursorRelation();
					fechaInicio = curR.isNull("fechainicio") ?  curR.valueBuffer("fecha") : curR.valueBuffer("fechainicio");
				} else {
					fechaInicio = util.sqlSelect("co_amortizaciones", "fechainicio", "codamortizacion = '" + cursor.valueBuffer("codamortizacion") + "'");
				}
			} else {
