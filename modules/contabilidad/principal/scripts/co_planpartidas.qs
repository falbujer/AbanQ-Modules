...
function interna_init()
{
	...
	if (cursor.modeAccess() == cursor.Edit) {
		...
		if (this.child("fdbTFactura").value().toString() == "Auto")
			this.iface.definirMemoriaC =  flcontppal.iface.pub_lugarPreMemoria("_NFAC_");
	...
	if (cursor.modeAccess() == cursor.Insert) {
		...
		this.child("chkDebe").setChecked(true);
		this.child("chkHaber").setChecked(false);
		this.child("btgDH").update();
	}
	...
	switch(fN) {
		...
		case "nsubcuenta":
				...
				if (!this.iface.bloqueoMemoriaS) {
					nombreDefinir = this.child("fdbNSubcuenta").value().toString().replace(/\s/g,"_");
					this.child("fdbNSubcuenta").setValue(nombreDefinir);
					this.iface.definirMemoriaS = flcontppal.iface.pub_putLugarPreMemoria
					(this.iface.definirMemoriaS, nombreDefinir, "Subcuenta");
				}
			break;
		case "nimporte":
				/** \C 
				...
				if (!this.iface.bloqueoMemoriaI) {
					nombreDefinir = this.child("fdbNImporte").value().toString().replace(/\s/g,"_");
					this.child("fdbNImporte").setValue(nombreDefinir);
					this.iface.definirMemoriaI = flcontppal.iface.pub_putLugarPreMemoria
					(this.iface.definirMemoriaI, nombreDefinir, "Importe");
				}
			break;
		case "nconcepto":
				/** \C 
				...
				if (!this.iface.bloqueoMemoriaC) {
					nombreDefinir = this.child("fdbNConcepto").value().toString().replace(/\s/g,"_");
					this.child("fdbNConcepto").setValue(nombreDefinir);
					this.iface.definirMemoriaC = flcontppal.iface.pub_putLugarPreMemoria
					(this.iface.definirMemoriaC, nombreDefinir, "Concepto");
				}
			break;
		case "nbaseimponible":
				/** \C 
				...
				if (!this.iface.bloqueoMemoriaBI) {
					nombreDefinir = this.child("fdbNBaseImponible").value().toString().replace(/\s/g,"_");
					this.child("fdbNBaseImponible").setValue(nombreDefinir);
					this.iface.definirMemoriaBI = flcontppal.iface.pub_putLugarPreMemoria
					(this.iface.definirMemoriaBI, nombreDefinir, "Importe");
				}
			break;
		case "ncontrapartida":
				/** \C 
				...
				if (!this.iface.bloqueoMemoriaCt) {
					nombreDefinir = this.child("fdbNContrapartida").value().toString().replace(/\s/g,"_");
					this.child("fdbNContrapartida").setValue(nombreDefinir);
					this.iface.definirMemoriaCt = flcontppal.iface.pub_putLugarPreMemoria
					(this.iface.definirMemoriaCt, nombreDefinir, "Subcuenta");
				}
			break;
		case "nimporteme":
				/** \C 
				...
				if (!this.iface.bloqueoMemoriaIme) {
					nombreDefinir = this.child("fdbNImporteme").value().toString().replace(/\s/g,"_");
					this.child("fdbNImporteme").setValue(nombreDefinir);
					this.iface.definirMemoriaIme = flcontppal.iface.pub_putLugarPreMemoria
					(this.iface.definirMemoriaIme, nombreDefinir, "Importe");
				}
			break;
