...
class oficial extends interna {
	...
	function accionesFinPaso(curPlanPartida) {
		return this.ctx.oficial_accionesFinPaso(curPlanPartida);
	}
...
function interna_validateForm():Boolean
{
	...
	if ( numPaso == totalPrepartidas - 1 ) {
		...
		for ( contador = 0; contador < totalPrepartidas; contador++) {
			...
			var elvalordefactura = "";
			var tipoImporte = curPlanPartidas.valueBuffer("tbaseimponible");
			switch( tipoImporte) {
				case "Pedir": {
				if (flcontppal.iface.pub_lugarPreMemoria("conceptoanteriorx") >= 0)
						elvalordefactura = flcontppal.iface.pub_getPreMemoria("conceptoanteriorx");
				
		// 			if (flcontppal.iface.pub_lugarPreMemoria("numeroseriex") >= 0)
		// 				elvalordefactura = flcontppal.iface.pub_getPreMemoria("numeroseriex");
		// 			if (elvalordefactura != "")
		// 				elvalordefactura = elvalordefactura + "/";
		// 			if (flcontppal.iface.pub_lugarPreMemoria("numerofacturax") >= 0)
		// 				elvalordefactura = elvalordefactura + flcontppal.iface.pub_getPreMemoria("numerofacturax");

		// 			valorFinal = curPlanPartidas.valueBuffer("concepto");
		// 			valorFinal = valorFinal.replace(/_NFAC_/g, elvalordefactura);
					curPartidas.setModeAccess(curPartidas.Edit);
					curPartidas.refreshBuffer();
					if ( contador == totalPrepartidas - 1 ) {
						this.child("fdbConcepto").setValue(elvalordefactura);
					} else {
						curPartidas.setValueBuffer("concepto", elvalordefactura);
						if (curPartidas.commitBuffer()) {
							commitOk = true;
						}
					}
				...
				case "Último": {debug("entra ultimo");
					var  esteIdPartida = curPartidas.valueBuffer("idpartida");
					var idPartida = AQUtil.sqlSelect("co_partidas","max(idpartida)","idasiento = " + cursor.valueBuffer("idasiento") + " and idpartida <> " + esteIdPartida);
					if(idPartida) {
						elvalordefactura = AQUtil.sqlSelect("co_partidas","concepto","idpartida = " + idPartida);
						curPartidas.setModeAccess(curPartidas.Edit);
						curPartidas.refreshBuffer();
						if ( contador == totalPrepartidas - 1 ) {
							this.child("fdbConcepto").setValue(elvalordefactura);
			...
			if ( contador == totalPrepartidas - 1 ) {
				...
				curPartidas.setValueBuffer("documento", valorFinal);
				if (curPartidas.commitBuffer()) {
					commitOk = true;
				}
			}
...
function oficial_bufferChanged(fN)
{
	...
	switch(fN) {
		...
		case "serie":
			...
			\end */
			this.iface.bloqueo = true;
			flcontppal.iface.pub_putPreMemoria("numeroseriex", this.child("fdbSerie").value());
			this.iface.bloqueo = false;
			break;
		...
		case "factura":
			...
			\end */
			this.iface.bloqueo = true;
			flcontppal.iface.pub_putPreMemoria("numerofacturax", this.child("fdbFactura").value());
			this.iface.bloqueo = false;
			break;
...
function oficial_establecerPaso()
{
	...
	if (this.iface.esIVA == true) {
		...
		//////////////// LA SERIE ///////////////
		valorFinal = curPlanPartidas.valueBuffer("codSerie");
		this.child("fdbSerie").setValue(valorFinal);
		this.iface.bufferChanged("serie"); // Memoriza la serie en numeroseriex
		...
		if (datoTipo == "Auto") {
			this.child("fdbFactura").setDisabled(true);
			this.child("fdbSerie").setDisabled(true);
			// gggg falta saber el siguiente número de factura
			valorFinal = "4"; // <---- CAMBIAR por función con siguiente factura de la serie
			this.child("fdbFactura").setValue(valorFinal);
			this.iface.bufferChanged("factura"); // También memoriza el número de factura en numerofacturax
		}
	...
	if (!this.iface.accionesFinPaso(curPlanPartidas)) {
		return false;
	}
...
/** \D Función a sobrecargar. Permite que algunas extensiones realicen acciones complementarias antes de finalizar el paso
\end */
function oficial_accionesFinPaso(curPlanPartida)
{
	return true;
}
