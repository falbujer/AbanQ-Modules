...
class oficial extends interna {
	...
	function bdBorrarDocRepo(codigo) {
		return this.ctx.oficial_bdBorrarDocRepo(codigo);
	}
	...
	function dameCxBdDocs() {
		return this.ctx.oficial_dameCxBdDocs();
	}
	function conectaBdDocs() {
		return this.ctx.oficial_conectaBdDocs();
	}
	...
	function valorTablaPlantilla(codTipo:String, campos:String, eDoc:FLDomElement):Array {
...
class ifaceCtx extends head {
	...
	function pub_valorTablaPlantilla(codTipo:String, campos:String, eDoc:FLDomElement):Array {
...
function oficial_borrarDocRepo(codigo:String):Boolean
{
	...
	if (this.iface.tipoRepositorio_ == "Base de datos")
		return this.iface.bdBorrarDocRepo(codigo);
...
function oficial_dameCxBdDocs()
{
  var _i = this.iface;
	if (_i.cxBdRepo_ == "undefined") {
		return false;
	}
	var nombreBd = _i.cxBdRepo_.nombre;
	if (nombreBd == "default") {
		return nombreBd;
	}
	...
	if (dbDocs) {
		if (dbDocs.connectionName() != nombreBd ||!dbDocs.isOpen()) {
			if (!_i.conectaBdDocs()) {
				return false;
			}
		}
...
function oficial_conectaBdDocs()
{
	debug("oficial_conectaBdDocs...");
  var _i = this.iface;
  if (_i.cxBdRepo_ == "undefined") {
		return false;
	}
  ...
  var datosConexion = "";
  var nombreBD = _i.cxBdRepo_.nombrebd;
  datosConexion += "\n" + sys.translate("Base de datos %1").arg(nombreBD);
  var host = _i.cxBdRepo_.servidor;
  datosConexion += "\n" + sys.translate("Servidor %1").arg(host);
  var driver = _i.cxBdRepo_.driver;
  datosConexion += "\n" + sys.translate("Driver %1").arg(driver);
  var puerto = _i.cxBdRepo_.puerto;
  datosConexion += "\n" + sys.translate("Puerto %1").arg(puerto);

  if (!driver || !nombreBD || !host) {
    MessageBox.warning(sys.translate("Debe indicar los datos de conexión a la base de datos Central en la opción Configuración de este módulo"), MessageBox.Ok, MessageBox.NoButton);
    return false;
  }
  ...
  var usuario = _i.cxBdRepo_.usuario;
  var password = _i.cxBdRepo_.contrasena;
	var conName = _i.cxBdRepo_.nombre;
...
function oficial_bdSubirDocumento(curD, comentario, rutaDoc)
{
	...
	var cxBd = _i.dameCxBdDocs();
	if (!cxBd) {
		MessageBox.warning(sys.translate("No hay conexión con la base de datos de documentos"), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton, "AbanQ");
		return false;
	}
	if (cxBd == "default") {
...
debug("oficial_subirDocumento " + this.iface.tipoRepositorio_);
	switch (this.iface.tipoRepositorio_) {
		case "Base de datos": {
			return this.iface.bdSubirDocumento(curDocumento, comentario, rutaDoc);
			break;
		}
		case "Directorio": {
...
function oficial_bdObtenerDocumento(codDocumento, pathDirectorio, version, revision, rutaRepositorio)
{
	...
	var idVer = AQUtil.sqlSelect("gd_documentos d INNER JOIN gd_versionesdoc vd ON d.iddocumento = vd.iddocumento", "vd.idversion", "d.codigo = '" + codDocumento + "'", "gd_documentos", cxBd);
	if (!idVer) {
		return false;
	}
	/** Bytearray */
...
function oficial_gestionDocumentalOn(container:Object, datosGD:Array):Boolean
{
	...
	if(flfactppal.iface.pub_extension("envio_mail")) {
		try {
			sys.connectSS(this.iface.container_.child("tbnEnviarMailGD"), "clicked()", this, "iface.enviarDocEmail");
		} catch (e) {
			if (this.iface.container_.child("tbnEnviarMailGD")) {
				connect(this.iface.container_.child("tbnEnviarMailGD"), "clicked()", this, "iface.enviarDocEmail");
			}
		}
	}
	else {
		if (this.iface.container_.child("tbnEnviarMailGD")) {
			this.iface.container_.child("tbnEnviarMailGD").close();
		}
	}
...
function oficial_verificarConfiguracion():Boolean
{
	...
	_i.urlRepositorio_ = util.sqlSelect("gd_config", "urlrepositorio", "1 = 1");
	if (_i.tipoRepositorio_ == "Base de datos") {
		var datosCx = _i.ejecutarQry("gd_config", "usarbdlocal,servidor,puerto,contrasena,nombrebd,usuario,driver", "1 = 1");
		if (datosCx.result != 1) {
			MessageBox.warning(sys.translate("No tiene definidos los parámetros de conexión a la base de datos de documentos"), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton, "AbanQ");
			return false;
		}
		_i.cxBdRepo_ = datosCx;
		_i.cxBdRepo_.nombre = _i.cxBdRepo_.usarbdlocal ? "default" : "BD_REPO";
	} else {
...
function oficial_bdBorrarDocRepo(codigo)
{
	var _i = this.iface;
