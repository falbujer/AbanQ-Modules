		...
		switch(nodo.attributeValue("class")) {
			...
			xmlReport += "\n\n";
				...
				switch(nodo.attributeValue("class")) {
					...
					break;
					case "rpPageHeader":
						xmlReport += "<PageHeader";
					break;
					case "rpPageFooter":
						xmlReport += "<PageFooter";
					break;
				}
									...
									case "styleSheet":
										...
										for(iE = 0; iE < arrayEstilos.length; iE++) {
											...
											if (estilo.search("background-color") > -1) {
												color = "";
												if (estilo.search("TRANSPARENT") > -1) {
										...
										for (l = 0; l < listaNodos3.length(); l++) {
											...
											switch(nodo4.nodeName()) {
												...
												case "width":
													...
													lineWidth = (lineWidth % 2 == 1 ? lineWidth + 1 : lineWidth);
												break;
												
												case "height":
													lineHeight = parseFloat(nodo4.toElement().text());
													lineHeight = (lineHeight % 2 == 1 ? lineHeight + 1 : lineHeight);
												break;
