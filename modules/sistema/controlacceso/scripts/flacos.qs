...
function oficial_setObjectName()
{
  ...
  switch (this.iface.tipo) {
    ...
    case "mainwindow": {
      ...
      if (verBuild >= 36032 && formRecordflacs.iface.desActual.endsWith("container")) {
        ...
        if (container != undefined) {
          ...
          if (list.isValid()) {
            ...
            while ((obj = list.current()) != undefined) {
              ...
              if (obj.name == "unnamed")
                continue;
              formRecordflacs.iface.flactions.push(obj.name);
debug("Incluyendo " + obj.name + " - " + obj.text + " - " + obj.menuText);
      ...
      if (isContainer) {
        this.iface.desActual = valor;
        this.child("fdbDescripcion").setValue(valor);
