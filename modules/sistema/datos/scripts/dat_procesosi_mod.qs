...
class oficial extends interna {
  ...
  function ordenImportacion(listaTablas) : Array {
    return this.ctx.oficial_ordenImportacion(listaTablas);
  }
  function obtenerRelaciones(tabla) : String {
    return this.ctx.oficial_obtenerRelaciones(tabla);
  }
  function moduloTabla(tabla) : String {
    return this.ctx.oficial_moduloTabla(tabla);
  }
  function vaciarModulo(listaOrdenada) : Boolean {
    return this.ctx.oficial_vaciarModulo(listaOrdenada);
  }
  function consultarDependencias(tablas) : Boolean {
    return this.ctx.oficial_consultarDependencias(tablas);
  }
}
//// OFICIAL /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
class ifaceCtx extends head {
  ...
  function pub_obtenerRelaciones(tabla) : String {
    return this.obtenerRelaciones(tabla);
  }
}
...
function oficial_importar() {
  ...
  if (!dir || dir == "" || !idModulo || idModulo == "") return;

  if (!File.isDir(dir)) {
    MessageBox.critical(util.translate("scripts", "El directorio indicado no es válido"), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
    return;
  }
  ...
  var nL:Number = fldatosppal.iface.pub_lineasTablas(listaTablas, dir);
  if (nL == "error") {
	  return;
  }
  ...
  for (var i = 0; i < listaOrdenada.length; i++) {
    ...
    if (formRecorddat_procesosi_tab.iface.pub_importarTabla(listaOrdenada[i], dir + listaOrdenada[i] + ".csv", true, util) == "cancel") {
  ...
  while (!todasImportadas) {
    ...
    for (var i = 0; i < listaTablas.length; i++) {
      ...
      for (var j = 0; j < relacionesTabla.length; j++) {
        ...
        case "actualModulo":
          if (!datosTablas[relacionesTabla[j]]["importada"]) importarTabla = false;
          break;

        case "noModulo":
        case "otroModulo":
          if (tablasOtrosModulos.search(relacionesTabla[j]) < 0) tablasOtrosModulos += "\n      " + relacionesTabla[j]
          break;
        }
      }
  ...
  for (var i = 0; i < listaRelaciones.length(); i++) {
    ...
    if (nodoRelacion.namedItem("card")) {
      if (nodoRelacion.namedItem("card").toElement().text() == "M1") {
        ...
        if (nomTablaRel != tabla && nomTablaRel != "flmodules") stringRelaciones += nomTablaRel + this.iface.sep;
      }
    }
  }
...
function oficial_moduloTabla(tabla) : String {
  ...
  var modTabla: String = util.sqlSelect("flfiles", "idmodulo", "nombre='" + tabla + ".mtd'");
