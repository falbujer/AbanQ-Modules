...
class oficial extends interna
{
  ...
  {
    return this.ctx.oficial_ejecutarFuncion();
  }
  function generarCSVs()
  {
    return this.ctx.oficial_generarCSVs();
  }
  function crearFicheroAsientos()
  {
    return this.ctx.oficial_crearFicheroAsientos();
  }
  function crearFicheroPC_Sub()
  {
    return this.ctx.oficial_crearFicheroPC_Sub();
  }
  function crearFicheroPC_SubCOD()
  {
    return this.ctx.oficial_crearFicheroPC_SubCOD();
  }
  function actualizarSubcuentas()
  {
    return this.ctx.oficial_actualizarSubcuentas();
  }
  function actualizarNumerosFacturacion()
  {
    return this.ctx.oficial_actualizarNumerosFacturacion();
  }
  function actualizarValoresFacturacion()
  {
    return this.ctx.oficial_actualizarValoresFacturacion();
  }
  function actualizarValorFacturacion(tabla, tablaLineas, campoID, msg)
  {
    return this.ctx.oficial_actualizarValorFacturacion(tabla, tablaLineas, campoID, msg);
  }
  function actualizarSecuencia(tablaOrigen, campoSecuencia, ejercicio, codSerie)
  {
    return this.ctx.oficial_actualizarSecuencia(tablaOrigen, campoSecuencia, ejercicio, codSerie);
  }
  function actualizarIVA(tipoTabla, msg)
  {
    return this.ctx.oficial_actualizarIVA(tipoTabla, msg);
  }
  function actualizarLineasIva(tipoTabla, idFactura)
  {
    return this.ctx.oficial_actualizarLineasIva(tipoTabla, idFactura);
  }
  function procesarPlazosPago()
  {
    return this.ctx.oficial_procesarPlazosPago();
  }
  function procesarComentarios()
  {
    return this.ctx.oficial_procesarComentarios();
  }
  function comentariosTabla(ficheroCSV, tabla, indCampoClaveF, indCampoComF, campoClaveT, campoComT, esFacturacion)
  {
    return this.ctx.oficial_comentariosTabla(ficheroCSV, tabla, indCampoClaveF, indCampoComF, campoClaveT, campoComT, esFacturacion);
  }
  function codigoFacturacion(tabla, campos)
  {
    return this.ctx.oficial_codigoFacturacion(tabla, campos);
  }
  function crearFicheroRappels()
  {
    return this.ctx.oficial_crearFicheroRappels();
  }
  function completarBancos()
  {
    return this.ctx.oficial_completarBancos();
  }
  function completarBancosTabla(fichero, indCampoEntidad, indCampoNombre) : Number {
    return this.ctx.oficial_completarBancosTabla(fichero, indCampoEntidad, indCampoNombre);
  }
  function actualizarSaldos()
  {
    return this.ctx.oficial_actualizarSaldos();
  }
  function completarFyR()
  {
    return this.ctx.oficial_completarFyR();
  }
  function completarFacturas()
  {
    return this.ctx.oficial_completarFacturas();
  }
  function completarRecibos()
  {
    return this.ctx.oficial_completarRecibos();
  }
  function actualizarSubcuentasCli()
  {
    return this.ctx.oficial_actualizarSubcuentasCli();
  }
  function actualizarSubcuentasProv()
  {
    return this.ctx.oficial_actualizarSubcuentasProv();
  }
  function trocearFichero()
  {
    return this.ctx.oficial_trocearFichero();
  }
  function familiasDesdeArt()
  {
    return this.ctx.oficial_familiasDesdeArt();
  }
  function rellenarSubcuentasCliProv()
  {
    return this.ctx.oficial_rellenarSubcuentasCliProv();
  }
  function VBACsvToAbanqCsv()
  {
    this.ctx.oficial_VBACsvToAbanqCsv();
  }
  function comprobarAsientos()
  {
    this.ctx.oficial_comprobarAsientos();
  }
}
//// OFICIAL /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
function oficial_ejecutarFuncion()
{
  ...
  switch (funcion) {
    ...
    case "familiasDesdeArt": {
      this.iface.familiasDesdeArt();
      break;
    }

    case "rellenarSubcuentasCliProv": {
      this.iface.rellenarSubcuentasCliProv();
      break;
    }

    case "VBACsvToAbanqCsv": {
      this.iface.VBACsvToAbanqCsv();
      break;
    }

    case "comprobarAsientos": {
      this.iface.comprobarAsientos();
      break;
    }

    default: {
			var f = new Function("", "return formdat_procesosesp.iface." + funcion + "()");
			try {
				f();
			} catch(e) {
				MessageBox.warning(sys.translate("La función %1 ha fallado o no existe: ").arg(funcion) + e, MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
			}
		}
  }
}
...
function oficial_generarCSVs()
{
  ...
  util.createProgressDialog(util.translate("scripts", "Creando ficheros CSV..."), listaDBF.length);
  util.setProgress(1);
  var paso: Number = 0;
  var pasoF: Number = 0;
  ...
  MessageBox.information(util.translate("scripts", "Proceso finalizado. Ficheros creados: " + pasoF),
                         MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
}
...
function oficial_crearFicheroAsientos()
{
  ...
  if (!File.exists(fichero)) {
    MessageBox.warning(util.translate("scripts", "No existe el fichero ") + fichero,
                       MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
    return 0;
  }
  ...
  try {
    ...
    MessageBox.warning(util.translate("scripts", "Imposible abrir el fichero ") +
                       fichAsientos.name +
                       util.translate("scripts", " para escritura\nCompruebe que la ruta es válida y que los permisos de escritura son correctos"),
                       MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
    return false;
  }
  ...
  util.createProgressDialog(util.translate("scripts", "Creando fichero de asientos..."), numLineas);
  util.setProgress(1);

  fichDiario.open(File.ReadOnly);
  if (!fichDiario.eof) linea = fichDiario.readLine();
  ...
  MessageBox.information(util.translate("scripts", "Proceso finalizado. Asientos registrados: ") + pasoA + util.translate("scripts", "\n\nFichero de asientos creado:\n") + fichAsientos.fullName, MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
}
...
function oficial_actualizarSubcuentas()
{
  ...
  if (numSubcuentas == 0) {
    MessageBox.information(util.translate("scripts", "No hay subcuentas para actualizar. No es necesario este proceso"), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
    return;
  }
...
function oficial_actualizarNumerosFacturacion()
{
  ...
  MessageBox.information(util.translate("scripts", "Proceso finalizado. Actualizadas las series del ejercicio ") + ejercicio, MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
}
...
function oficial_actualizarValorFacturacion(tabla, tablaLineas, campoID, msg)
{
  ...
  util.createProgressDialog(util.translate("scripts", "Procesando ") + msg + "...", numDocs);
  util.setProgress(1);
  var paso: Number = 0;
  ...
  q.setSelect("SUM(pvptotal), SUM(pvptotal*iva/100), SUM(pvptotal*recargo/100)");
  ...
  while (curTabla.next()) {
    ...
    curTabla.setValueBuffer("totalrecargo", q.value(2));
    total = q.value(0) + q.value(1) + q.value(2);
...
function oficial_actualizarLineasIva(tipoTabla, idFactura)
{
  ...
  while (curLineasFactura.next()) {
    ...
    if (ivaAnt != iva) {
      ...
      if (codImpuesto) curLineaIva.setValueBuffer("codimpuesto", codImpuesto);
...
function oficial_comentariosTabla(ficheroCSV, tabla, indCampoClaveF, indCampoComF, campoClaveT, campoComT, esFacturacion)
{
  ...
  util.createProgressDialog(util.translate("scripts", "Procesando comentarios de ") + tabla + "...", totalSteps);
  util.setProgress(1);
  var paso: Number = 0;
  ...
  for (i = 1; i < lineas.length; i++) {
    ...
    if (esFacturacion) clave = this.iface.codigoFacturacion(tabla, linea);
    else clave = linea[indCampoClaveF];
...
function oficial_codigoFacturacion(tabla, campos)
{
  ...
  switch (tabla) {
    ...
    case "albaranesprov":
      numero = campos[0];
      codigo = fldatosppal.iface.pub_cerosIzquierda(codEjercicio, 4) + fldatosppal.iface.pub_cerosIzquierda(codSerie, 2) + fldatosppal.iface.pub_cerosIzquierda(numero, 6);
      break;
...
function oficial_crearFicheroRappels()
{
  ...
  if (!fichIntervalos.eof) linea = fichIntervalos.readLine();
  ...
  MessageBox.information(util.translate("scripts", "Proceso finalizado. Rappels registrados: ") + pasoR + util.translate("scripts", "\n\nFichero de rappels creado:\n") + fichRappels.fullName, MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
}
...
function oficial_completarBancos()
{
  ...
  MessageBox.information(util.translate("scripts", "Proceso finalizado. Nuevos bancos insertados: ") + numNuevos, MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
}
...
function oficial_completarBancosTabla(fichero, indCampoEntidad, indCampoNombre) : Number {
  ...
  util.createProgressDialog(util.translate("scripts", "Actualizando bancos..."), numLineas);
  util.setProgress(1);
...
function oficial_actualizarSaldos()
{
  ...
  util.createProgressDialog(util.translate("scripts", "Calculando saldos de las Subcuentas..."), numSubcuentas);
...
function oficial_completarFyR()
{
  ...
  MessageBox.information(util.translate("scripts", "Proceso finalizado\n\nFacturas actualizadas: ") + numFacturas + util.translate("scripts", "\nRecibos actualizados: ") + numRecibos, MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
  return true;
}
...
function oficial_completarFacturas()
{
  ...
  util.createProgressDialog(util.translate("scripts", "Actualizando facturas..."), numLineas);
  util.setProgress(1);
...
function oficial_completarRecibos()
{
  ...
  util.createProgressDialog(util.translate("scripts", "Actualizando recibos..."), numLineas);
  util.setProgress(1);
...
function oficial_crearFicheroPC_Sub()
{
  ...
  util.createProgressDialog(util.translate("scripts", "Creando ficheros de clientes y proveedores por subcuenta..."), numLineas);
  util.setProgress(1);
  ...
  MessageBox.information(util.translate("scripts", "Proceso finalizado. Proveedores y clientes registrados: ") + pasoA + util.translate("scripts", "\n\nFichero de clientes por subcuenta creado:\n") + fichCliSub.fullName + util.translate("scripts", "\nFichero de proveedores por subcuenta creado:\n") + fichProvSub.fullName, MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
}
...
function oficial_actualizarSubcuentasCli()
{
  ...
  while (cursor.next()) {
    ...
    if (!curScta.first()) {
      ...
      while (curScta.next()) {
        var res = MessageBox.warning(util.translate("scripts", "No se ha encontrado subcuenta para el cliente " + nombre + ".\nSin embargo parece que esta se le parece :\n " + curScta.valueBuffer("codsubcuenta") + "  " + curScta.valueBuffer("descripcion") + "\n\n¿La utilizo para actualizar este cliente?"), MessageBox.No, MessageBox.Yes, MessageBox.NoButton);
...
function oficial_trocearFichero()
{
  ...
  if (!fichPadre.eof) encabezados = fichPadre.readLine();
  ...
  MessageBox.information(util.translate("scripts", "Proceso finalizado. Ficheros creados: ") + numHijo, MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
}

function oficial_familiasDesdeArt()
{
  var util: FLUtil = new FLUtil();
  var paso: Number = 0;
  var codFamilia: String;
  var sep = "ð";
  var faltan: String = sep;

  var curArt: FLSqlCursor = new FLSqlCursor("articulos");
  var curFam: FLSqlCursor = new FLSqlCursor("familias");

  curArt.select();
  while (curArt.next()) {
    codFamilia = curArt.valueBuffer("referencia");
    codFamilia = codFamilia.toString().left(2);
    curFam.select("codfamilia = '" + codFamilia + "'");
    if (!curFam.first()) {
      if (faltan.indexOf(codFamilia) == -1) faltan += codFamilia + sep;
    }
  }

  var familias: Array = faltan.split(sep);
  for (var i: Number = 0; i < familias.length; i++) {
    codFamilia = familias[i];
    curFam.setModeAccess(curFam.Insert);
    curFam.refreshBuffer();
    curFam.setValueBuffer("codfamilia", codFamilia);
    curFam.setValueBuffer("descripcion", "Familia " + codFamilia);
    curFam.setValueBuffer("coddescuento", "100");
    curFam.commitBuffer();

    curArt.setModeAccess(curArt.Edit);
    curArt.refreshBuffer();
    curArt.setValueBuffer("codfamilia", codFamilia);
    curArt.commitBuffer();
  }
...
/** Rellena las subcuentas de co_subcuentascli y co_subcuentasprov para el ejercicio actual
*/
function oficial_rellenarSubcuentasCliProv()
{
  var _i = this.iface;
  ...
  var util = new FLUtil;
  var numDSubcuenta = util.sqlSelect("ejercicios", "longsubcuenta", "codejercicio = '" + codEjercicio + "'");
  var i = 0;
  ...
  util.setProgress(0);
  var codSubcuentaC, lSubcuenta, codSubcuenta, numCeros, idSubcuenta;
  while (qryClientes.next()) {
    codSubcuentaC = qryClientes.value("codsubcuenta");
    lSubcuenta = codSubcuentaC.length;
    codSubcuenta = codSubcuentaC.left(3);
    numCeros = numDSubcuenta - lSubcuenta;
    if (numCeros >= 0) {
      for (var k = 0; k < numCeros; k++) {
        codSubcuenta += "0";
      }
      codSubcuenta += codSubcuentaC.right(lSubcuenta - 3);
    } else {
      codSubcuenta += codSubcuentaC.right(numDSubcuenta - 3);
    }

    idSubcuenta = flfactppal.iface.crearSubcuenta(codSubcuenta, qryClientes.value("nombre"), "CLIENT", codEjercicio);
    if (!idSubcuenta) {
      return false;
    }
    if (!flfactppal.iface.crearSubcuentaCli(codSubcuenta, idSubcuenta, qryClientes.value("codcliente"), codEjercicio)) {
      return false;
    }
    util.setProgress(i++);
  }
  ...
  while (qryProveedores.next()) {
    codSubcuentaC = qryProveedores.value("codsubcuenta");
    lSubcuenta = codSubcuentaC.length;
    codSubcuenta = codSubcuentaC.left(3);
    numCeros = numDSubcuenta - lSubcuenta;
    if (numCeros >= 0) {
    ...
    idSubcuenta = flfactppal.iface.crearSubcuenta(codSubcuenta, qryProveedores.value("nombre"), "PROVEE", codEjercicio);
    if (!idSubcuenta) {
      return false;
    }
    if (!flfactppal.iface.crearSubcuentaProv(codSubcuenta, idSubcuenta, qryProveedores.value("codproveedor"), codEjercicio)) {
      return false;
    }
    util.setProgress(i++);
  }
...
function oficial_VBACsvToAbanqCsv()
{
  var dirSrc = FileDialog.getExistingDirectory(false, sys.translate("Directorio Origen"));
  if (!dirSrc)
    return;
  dirSrc = Dir.cleanDirPath(dirSrc);
  dirSrc = Dir.convertSeparators(dirSrc);
  Dir.current = dirSrc;

  var dirDst = FileDialog.getExistingDirectory(false, sys.translate("Directorio Destino"));
  if (!dirDst)
    return;
  ...
  for (var i = 0; i < listFiles.length; ++i) {
    ...
    fldatosppal.iface.pub_fileVBAToAbanqCsv(listFiles[i], dirDst + f.name, true);
  }

  sys.infoMsgBox(sys.translate("Proceso terminado"));
}

function oficial_comprobarAsientos()
{
  var codEjercicio = fldatosppal.iface.pub_ejercicioDefecto();
  var step = 0;

  var qry = new AQSqlQuery;
  qry.setSelect("idasiento");
  qry.setFrom("co_asientos");
  qry.setWhere("codejercicio='" + codEjercicio + "'");

  if (!qry.exec()) {
    sys.errorMsgBox(sys.translate("Se produjo un error en la consulta"));
    return;
  }

  AQUtil.createProgressDialog(
    sys.translate("Comprobando y actualizando importes de asientos..."), qry.size()
  );
  while (qry.next()) {
    flcontppal.iface.pub_comprobarAsiento(qry.value(0), false);
    AQUtil.setProgress(++step);
  }
  AQUtil.destroyProgressDialog();

  sys.infoMsgBox(sys.translate("Proceso finalizado. Asientos actualizados: ") + step);
}
//// OFICIAL /////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
