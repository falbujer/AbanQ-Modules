/***************************************************************************
                 tt_cargarmodulos.qs  -  description
                             -------------------
...
function interna_main()
{
	...
	if (File.exists(this.iface.pathLocal + this.iface.idFuncional + "/prueba"))
		dirModulos = this.iface.pathLocal + this.iface.idFuncional + "/prueba";
	else if (File.exists(this.iface.pathLocal + this.iface.idFuncional + "/modulos"))
		dirModulos = this.iface.pathLocal + this.iface.idFuncional + "/modulos";
	else {
		MessageBox.warning(util.translate( "scripts", "El directorio actual no contiene un subdirectorio llamado pruebas o modulos:\n") + this.iface.pathLocal + this.iface.idFuncional, MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton );
		return;
	}
	Dir.current = dirModulos;
	
	var resComando:Array = this.iface.ejecutarComando("find . -name *.mod");
	if (resComando.ok == false) {
		MessageBox.warning(util.translate( "scripts", "Error al buscar los módulos en el directorio:\n") + dirModulos, MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton );
		return;
	}
...
function oficial_cargarModulo(nombreFichero:String):Boolean
{
	...
	if ( xmlModule.setContent( f ) ) {
		...
		if ( aF.length > 7 )
			dependencias = dameValor( aF[ 7 ] ).split( / [ , ; ] / );
	}
